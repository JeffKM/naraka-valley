...
- [ ] **T5.5 — 멜 관계 보상축 (여우불 대응)**
  - 완료기준: 멜 호감도가 높을수록 카페 운영 이득(매출/효율 등)이 커지는 차이가 체감된다.
  - 의존: T5.4, T5.2 · 메모: `foxfire.gd` 틀(하트→이득 static 매핑, 세이브 무상태). 구체 이득 *내용*은 착수 시 확정(서랍). ADR-0004 속죄(돈으로 망침 → 카페를 굴림).
...
### Sprint 7 (~2주) — 직조 + ★통합 재미 게이트
- [ ] 낮↔밤 직조 — 낮(농사·미호)↔밤(경비·바나) 시간 분업, 자원·관계 합류 → [§2.8](./docs/design/game-loops.md) · 의존: Sprint 6
- [ ] 통합 슬라이스로 묶기 — 슬라이스 길이 재조정(14일 유지 or 확장, *grill로*) + 통합 마무리 화면
- [ ] **★ 통합 재미 게이트** — 세 루프가 어우러지는 슬라이스 위에서 "또 하고 싶나" 재판정(봇 `playtest_bot` 확장 + 직접 플레이). 통과 → Phase 2(에셋) / 실패 → 루프 재설계
- **▶ 플레이 산출물:** 농사·멜·바나가 낮↔밤으로 맞물리는 슬라이스가 완주되고, 재미를 판정한다.
