...
static func _obtainable_between(id: String, post_day: int, due_day: int) -> bool:
	...
	# ★[폴리시 R26 #3] 축은 «**게시일에 세계가 그 종을 쥐고 있는가**» 하나다(R25 #22의 교집합에서
	#   정정). 교집합은 막으려던 것(=다음 절기 전용 종)을 막으면서 **현 절기 종까지 같이 지웠다**:
	#   절기 마지막 날 28일 게시분은 `season(28)=X`·`season(29)=X+1`이라 X 전용 종이
	#   `X == X and X == X+1`로 걸러져, 그날 마당에 실제로 돋아 있어 주워서 그날 납품할 수 있는
	#   채집물이 후보에서 빠졌다 — 절기마다 하루씩(연 4일) 채집 갈래가 통째로 사라지고 작물만
	#   남았다(폴백은 작물이 늘 남아 안 걸리므로 조용한 손실이었다).
	#   ★ 하한선은 그대로다: R25가 든 «다음 절기 전용 종»은 게시일 세계에 한 톨도 없으므로 이
	#     한 항으로 정확히 거절된다(기한일 항은 애초에 하중이 없었고, 연언으로 쓰이면서 해가 됐다).
	#   ★ 위 덤불 갈래는 자기 창(피안 15~18 / 망연 8~11)을 [post_day, due_day] 전 구간으로 이미
	#     따로 재므로 이 줄과 축이 갈리지 않는다.
