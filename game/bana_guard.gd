...
#   들어가는데 요약은 여전히 `auto_block(hearts)`만 읽어, 관계 탭이 실효 횟수보다 한 마리
#   적게 말했다(결혼으로 얻은 보호를 확인할 창구가 화면 어디에도 없었다).
#   기본값 0이라 기존 호출부는 한 글자도 안 바뀐다.
static func summary(hearts: int, extra_block: int = 0) -> String:
	var ab := auto_block(hearts) + maxi(extra_block, 0)
