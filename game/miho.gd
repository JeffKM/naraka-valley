...
const LINES_INTRO := [
	...
	# ★[폴리시 R15] 옛 문구는 "저기 카페 보이지?"였다 — M1.4에서 카페가 나루 마을로 이주해 안식
	#   농원엔 외관이 한 줄도 안 그려지는데, 이 대화가 벌어지는 자리(MIHO_FIELD_TILE)는 안식 농원이라
	#   손가락질할 대상이 화면에 없었다. 방향 지시만 실제 지리에 맞춘다(다음 줄 "카페 가 봐"와 짝).
	"[talk]카페는 동쪽 길 건너 나루 마을에 있어. 거기 사장이 옥자야. 무섭게 생겼는데 안 무서워. …좀 무섭나?",
