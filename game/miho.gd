...
const LINES_WARMING := [
	"또 왔구나! 네가 오면 이 밭이 좀 더 환해지는 것 같아.",
	"있잖아… 난 살아서 불을 너무 사랑했어. 태우는 불 말이야.",
	"여기선 그 불로 싹을 틔워. 망치던 힘을, 살리는 데 쓰는 거지. 이상하지?",
]

# 하트 3+(가까운 사이): 더 깊은 속죄 토막 + 여우불 강화(T3.4) 떡밥.
const LINES_CLOSE := [
	"너랑 이렇게 얘기하는 시간이 좋아. 진짜로.",
	"내 여우불, 이젠 제법 말을 들어. 네 밭이라면 더 잘 자라게 해줄 수 있을 것 같아.",
	"속죄가 별건가… 어제보다 한 포기 더 살리면 되는 거지. 네 덕분에 그걸 배워.",
]

# 오늘 이미 일일 대화를 한 뒤 또 말 걸었을 때(점수 없음 — 대사만 가볍게 바뀐다).
const LINE_AGAIN := "오늘은 아까 봤잖아~ 그래도 얼굴 보니 좋네."
