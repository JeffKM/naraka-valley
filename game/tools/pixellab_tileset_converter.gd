...
# PixelLab → Godot Tileset Converter (Split Format)
# 출처: PixelLab MCP 공식 가이드(pixellab://docs/godot/wang-tilesets)를 그대로 벤더링.
# 변경점: output_path를 res:// 절대경로로(헤드리스 cwd 의존 제거). ADR-0001 허용 글루(변환 엔진 아님).
# 사용: godot --headless --path game -s res://tools/pixellab_tileset_converter.gd \
#         res://assets/tiles/grass_path_metadata.json res://assets/tiles/grass_path_image.png ...

var output_path = "res://assets/tiles/combined_terrain.tres"
var tile_size = 0
var terrains = {}
var tiles = []
...
func _init():
	...
	if tileset_pairs.is_empty():
		...
		print("Usage: godot --headless -s pixellab_tileset_converter.gd metadata1.json image1.png ...")
