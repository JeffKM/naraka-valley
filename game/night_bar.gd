...
#       ㉠ 재고 방어: raid_amount(약탈량) — ♡↑ → 잡귀가 훔치는 재고량↓. approach_secs(접근
#          시간)도 같은 축(♡↑ → 잡귀가 더 천천히 와 막을 여유↑). 둘 다 기본값에서 시작.
#       ㉡ 응대 보호: patience_secs(손님 인내심) — ♡↑ → 카운터 빈 사이 손님이 더 오래 버팀
#          (cafe.patience_secs와 같은 자리, 응대 실패 손실 방어). 기본값에서 시작.
#     ♡0이면 셋 다 기본값 = 바나 잠듦(밤은 거칠지만 base로 굴러감, ADR-0008 평평≠막힘).
#   - 범위 밖(후속): 바나 이중 보호 곱셈기 *주입*(T6.5) · 손님 종류·요구 다양성·창고 잡귀
#     자동 차단(2층) · 실제 전투(Phase 3). T6.4는 "막기 즉시 판정 + 응대 경쟁 + 이중 손실"까지만.
...
const DEFAULT_RAID := 1           # 잡귀 1마리 돌파 시 약탈 재고량 — ★seam ㉠: T6.5 보호가 줄임(약탈량↓)
