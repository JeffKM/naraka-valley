...
# ★[폴리시 R11] 세션 폐기 — `end_day`의 리셋만 취하고 **정산(closed)은 쏘지 않는다**. 로드(F9·
# 슬롯 전환)는 이 밤이 있었던 적 없는 시점으로 되감는 것이라, 그 밤의 약탈·매출·이탈을 합산해
# 알릴 자리가 없다(낚시 릴·체키·칵테일 세션이 로드에서 버려지는 것과 같은 규약 — 세션 로컬
# 상태는 되감기의 대상이 아니라 폐기의 대상이다).
#   왜 필요했나: 이 노드는 세이브 무상태라 로드가 한 줄도 안 건드렸는데, `_opened`·스폿 접근
#   타이머·`_auto_blocks_left`는 **로드를 그대로 통과했다**. 그래서 바를 연 밤에 F9를 누르면
#   되감긴 그날 19:00에 `tick`의 `_opened and _in_window`가 곧바로 참이 되어, 플레이어가 열지도
#   않은 바에서 잡귀가 인벤토리를 약탈했다(옵트인의 근간 — ADR-0010 #6 "안 열면 빈 밤" — 이
#   로드 한 번으로 무효). 바나 자동 차단 잔량도 이미 소모된 채였고, 취침 때 쏘는 정산 요약은
#   무효화된 매출·약탈까지 합산해 거짓말을 했다.
func abandon() -> void:
	_opened = false
	_was_active = false
	_raided = 0
