...
#   Weather는 세이브를 모르는 순수 파생 롤이다. 슬롯 기준이면 Weather는 day 하나로 답을 낸다.
# ⚠️ [폴리시 R19 #13] 종전 주석이 이어서 «부작용도 없다 — 누구도 손해 보지 않는 무해한 잉여»라고
#   단정했는데 **사실이 아니었다**: 혼불 바람(몹 1.5·드랍 1.5·희귀 2.0)·혼우(밭 자동 급수·입질
#   1.25)가 실효 이득이라, 비해금 슬롯을 덮으면 배너도 장식도 프리미엄도 없는 완전한 평일에서
#   그날치 하늘 이득만 사라졌다. 그래서 **개최 여부는 이제 Weather가 인자로 받는다**
#   (`Weather.weather_for_day(d, theme_open)`) — 이 술어는 여전히 슬롯만 보고 무상태를 지키되,
#   아는 쪽(main)이 해금까지 통과한 답을 들고 온다.
