...
# ★[S10-T2 / ADR-0069 결정 4] 레어크로우 획득처 ⑥ — 더비 특별 부상(세이브 1회 한정).
# ★ **보상 사다리(DERBY_REWARDS)에 한 칸으로 얹지 않은 근거**: 더비 원장은 *당일치*라 교환 횟수가
#   매 행사일 0으로 되감긴다(_roll_over). 사다리 칸으로 두면 해마다 같은 칸에서 또 나온다 — 세상에
#   한 마리뿐인 수집물이 매년 복제되는 셈이다. 그래서 사다리와 별개의 **누적 교환 문턱 + 1회 한정**
#   창구로 둔다(한정 이력은 main이 `market_bought` 원장에 적는다 — 절기 행사 한정 물품의 그 원장).
# ★ 문턱 3회(잠정) = 사다리를 끝까지 한 번 밟아 본 사람에게 준다는 뜻이다(태그 3개 ≈ 어획 9마리).
const DERBY_RARECROW := "rarecrow_6"
const DERBY_RARECROW_EXCHANGES := 3

# ★[S10-T2 / ADR-0069 결정 4] 레어크로우 획득처 ⑦ — 장원제 **장원(최고 등급)** 부상(세이브 1회 한정).
# 등급 보상(엽전·품질 비료)과 달리 반복 지급이 아니다: 장원을 한 번 따 본 적이 있는가가 조건이고,
# 그 뒤의 장원은 기존 보상만 준다(같은 이유 — 수집물은 복제되지 않는다).
const GRANGE_RARECROW := "rarecrow_7"
...
# ★[S10-T2] 이름은 야시장에서 왔지만 실체는 **"이 세이브에서 이미 받은 절기 행사 한정 물품 id 목록"**
#   이다 — 더비 ⑥·장원제 ⑦ 부상도 같은 원장에 적힌다(같은 성질의 이력에 세이브 키를 더 열지 않는다).
