...
# 납품 풀 = **깊이 게이트를 넘은 광물** 중 정가 상한 이하.
#   · 깊이 — `MineFloors.is_depth_gated`가 판다(단일 술어). R9가 그것을 신설하며 「소매 창구가
#     '무엇을 팔면 안 되나'를 물을 때 쓰는 단일 술어」라 적었고, 시련도 정확히 그 질문이다.
#   · 상한 300 — 명부금강(750)·오색혼옥(2000)은 들어오는 길이 좁아야 하는 물건이라([ADR-0063]
#     결정 2) 주간 의무 납품의 표적이 되면 그 희소성이 통째로 무너진다. 이 축은 깊이와 무관하게
#     그대로 남는다(둘은 다른 것을 막는다).
# ★[폴리시 R27 #21] **하한 50냥을 깊이 술어로 갈았다.** 옛 하한의 근거가 곧 깊이인데("지천의
#   것을 요구하면 심부름이 된다") 자[尺]가 판매가라 두 축의 답이 어긋났다:
#   ㉠ 넋알돌(50·floor_min 1)·넋수정(100·floor_min 1)이 풀에 들어, `mine_floors.gd:166`이 스스로
#      「전층 노드는 1층에서 손에 넣을 수 있는 물건」이라 적어 둔 그 둘로 시련이 성립했다
#      (「넋알돌 ×9」가 뽑히면 1층만 왕복해도 끝난다 = 위 문장이 그대로 깨진다).
#   ㉡ **나락철 광석**(100)이 들어왔다 — NODE_TABLE에 아예 없는 나락 전용 산출이자 도구 4티어의
#      최종 재료라, 보부상은 같은 물건을 이름으로 못 박아 뺀다(`peddler.gd:102` 「사지면 채광 깊이
#      곡선이 통째로 무의미해진다」). 시련은 완료 시 5~9개를 **소각**하므로 그 배제를 통과한 채
#      4티어 도구 한 자루 값(광석 25개)의 최대 36%를 태웠다. `is_depth_gated`는 표에 없는 종에
#      false를 돌려주므로 이 갈래가 함께 닫힌다.
#   ㉢ 반대로 황천금(25·41층)·유철(10·21층)은 깊이 게이트를 넘고도 값이 싸서 탈락했었다 — 이제 든다.
# ★ 분모를 손으로 안 적는다 — `ItemCatalog.MINERALS`·`MineFloors.NODE_TABLE`이 늘면 풀이 저절로 따라온다.
...
static func deliver_pool() -> Array:
	...
	for id in ItemCatalog.MINERALS:
		...
		if MineFloors.is_depth_gated(sid) and price <= DELIVER_PRICE_MAX:
			out.append(sid)
	out.sort()
	return out
