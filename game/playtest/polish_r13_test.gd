...
func _check_ship_bin_name() -> void:
	...
	# ★[폴리시 R27 #3] 회수 니들에 등급 앞머리가 붙었다 — R25 #14가 «한 등급만 되돌린 경우엔 그
	#   등급을 말한다»로 `_on_frame_takeback` 문구를 `%s%s`로 갈았다(투입 알림은 그때 이미 `%s%s`).
	#   재는 계약(«셋 다 '출하함'이라 부른다»)은 그대로고, 니들만 새 문구로 옮겨 적는다.
	_check("⑨c-pre 무대: 투입·회수·정산 알림 셋은 종전부터 '출하함'이었다(다수결의 근거)",
		_line_of("\"출하함에 %s%s %d개 (다음 아침 정산)\"") > 0
		and _line_of("\"출하함에서 %s%s %d개 회수\"") > 0
