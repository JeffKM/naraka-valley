...
func _initialize() -> void:
	...
	# ── 이전 stub이던 업화 갱도가 이제 *정상 복원*되는가(M5.2 — 8구역 전부 빌드, stub 0) ──
	# ★ M5.2 — 업화 갱도·나락이 빌드돼 stub이 0개다. M4.2까지 폴백 예시였던 업화 갱도는 이제 정상 복원된다
	#   (미빌드→폴백 경로는 ④ 미지 id가 계속 덮는다 — is_built=false면 홈베이스 폴백, 회귀 0).
	var sm2 := SaveManager.new()
	sm2.save_game({"region": RegionCatalog.EOPHWA_MINE, "indoor": "", "player_tile": Vector2i(20, 22)})
	sm2.free()
	var m6: Node = await _spawn_main()
	_check("⑤ 업화 갱도(이전 stub) 정상 복원(폴백 아님)", m6._region == RegionCatalog.EOPHWA_MINE)
