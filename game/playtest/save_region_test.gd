...
func _initialize() -> void:
	...
	# ── 미빌드 stub 구역(삼도천)도 미지와 같게 폴백되는가 ──
	var sm2 := SaveManager.new()
	sm2.save_game({"region": RegionCatalog.SAMDOCHEON, "indoor": "", "player_tile": Vector2i(2, 2)})
	sm2.free()
	var m6: Node = await _spawn_main()
	_check("⑤ 미빌드 stub 구역(삼도천) → 홈베이스 폴백", m6._region == RegionCatalog.HOME)
