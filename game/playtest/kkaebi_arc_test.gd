extends SceneTree
# ★[S9b-T1 / ADR-0068 결정 3·5·6] 깨비 풀 온보딩 + 소프트 게이트 ㉠ — 헤드리스 검증.
#
# 무엇을 보증하나:
...
#   ⑤ 여진 편지 — ♡2·♡3·♡4가 각기 실존하는 편지를 지목하고(♡1은 없음) 발신인이 깨비이며,
#      관문 성사가 큐에 넣고 다음 날 아침 도착한다.
#   ⑥ 절기 물음 4개 — 짝이 맞고, 주 첫날 첫 대화에 서며, **0점 계약**(선택 전후 점수·stage 불변).
#   ⑦ 생일 — 훅 본문 + Resident.BIRTHDAYS 배정 + main 경로가 물린다.
#   ⑧ 볼륨([ADR-0068] 결정 5 상한) — 대사 100~150줄 · 컷신 4 · 절기 4 · 편지 ≤3 · **spouse 4축**.
#   ⑨ 봉인 법칙([ADR-0068] 결정 6) — 금칙어 31어 스캔 0 + 조연 경계 구조 단언(다른 불의 주인을
#      지목하지 않는다 · 플레이어 죄목을 겨누지 않는다 · Affinity를 모른다).
#   ⑩ ★**소프트 게이트 ㉠** — 조연 로스터의 ♡3 진급은 메인 3인 중 1인 이상 stage≥3일 때만
#      성사된다. 미충족 = 기존 `pending_promotion` 대기(점수 손실 0·비트 0·편지 0), 충족 후
#      다시 대화하면 그 자리에서 성사. ♡1·♡2·♡4는 게이트 밖이고 점주(T2)도 대상이 아니다.
#   ⑪ 개통 계약 — confession/divorce/spouse 훅이 먼저 서 있었고, **S9b-T6이
#      main `ROMANCE_OPEN`에 깨비를 넣어 열었다**(콘텐츠 재작업 0).
#
# 실행: ./run_tests.sh kkaebi_arc   (헤드리스는 반드시 game/에서 · 순차)
...
func _run_checks() -> void:
	print("══ S9b-T1 깨비 풀 온보딩 · 소프트 게이트 ㉠ 검증 ══")
	...
	_check("①b 표시명·세이브 키·관계 트랙·선물 채널",
		...
		and r.save_key == "kkaebi_affinity" and r.affinity != null and r.can_gift)
	...
	_check("①d 초상화 stem = kkaebi(도트 버스트 · S9b-T9 아트 패스)", r.portrait_stem == "kkaebi")
	# 집 배정 = 주민 집 6(index 5). 아침 자리가 그 집 문 바로 아래 칸이어야 동선이 성립한다.
	var house_door: Vector2i = m.RESIDENT_HOUSE_DOORS[5]
	...
	_check("①h 집 6은 동편 주거다(강변 2채 = 세레나 예약분을 안 쓴다)", _house_is_east(m, 5))
	_check("①i 모찌 집(4)과 다른 집이다(1인 1채)",
		m.RESIDENT_HOUSE_DOORS[5] != m.RESIDENT_HOUSE_DOORS[3])
	...
	_check("①j 선호 선물 테이블(러브 5 · 헤이트 2 · 전부 실존·건넬 수 있음)",
		...
		and _prefs_valid())
	_check("①k 무쇠 헤이트(도깨비 설화의 금기 — 풀무 러브와 정확히 반대편)",
		GiftPrefs.tier_of(KID, ItemCatalog.INGOT_YUCHEOL) == GiftPrefs.HATE
	...
	# 실제 재생 — ♡3은 소프트 게이트가 필요하므로 여기서 먼저 열어 둔다(게이트 자체는 ⑩이 잰다).
	...
	for t in [2, 3, 4]:
		if Mailbox.sender_of(String(who.heart_gate_letter(t))) != NAME_KO:
			sender_ok = false
	_check("⑤b 세 통 다 발신인이 깨비다", sender_ok)
	...
	_check("⑦b 생일 배정이 살아 있다(달력 단일 출처 · 유화절)", b.size() == 2 and int(b[0]) == 1)
	...
	# ── ⑨ 봉인 법칙([ADR-0068] 결정 6) ─────────────────────────────────────
	print("── ⑨ 봉인 법칙 ──")
	# 기존 아크 스위트(미호·멜·바나·T1 2인)의 금칙어를 **합집합으로 상속**하고, 결정 6의 중심
	# 진실 4금지(옥자 희생 / 기억 봉인 / 마녀=연인 / 플레이어 죄목)를 어휘로 못 박는다.
	...
	_check("⑨b ♡3은 **자기 파편**이다(불을 숨긴 것과 못 끈 것까지)",
		g3.contains("숨겨") and g3.contains("끌 수가 없"))
	_check("⑨c ★다른 불의 주인을 지목하지 않는다(코러스 = 자기 파편만)",
		g3.contains("누구 건지는 나도 몰라")
		and not g3.contains("미호") and not g3.contains("여우"))
	_check("⑨d ★그 밤의 인물을 호명하지 않는다(옥자·약방·플레이어 죄목 0)",
		not g3.contains("옥자") and not g3.contains("약방"))
	var g4 := _joined(who.GATE_HEART_4)
	_check("⑨e ♡4는 **자기 생전 죄까지만**(장난으로 진심을 못 보인 죄 · 못 한 사과)",
		g4.contains("장난으로") and g4.contains("사과"))
	_check("⑨f ♡4가 플레이어의 죄를 겨누지 않는다(메인 3인 조각의 독점 영역)",
	...
	#   (luck_forecast_test ⑦a 관례를 아크 다섯 스위트만 안 갖고 있었다).
	_check("⑨g-pre %s.gd를 실제로 읽었다(부정 단언이 공허하지 않다 — 주석 제외 %d자)"
			% [KID, _src().length()],
	...
	_check("⑨h 불을 메카닉으로 쓰지 않는다(residents.md 가드레일 — effect_fn 0 · 곱셈기 어휘 0)",
		not r.effect_fn.is_valid() and not _src().contains("effect_fn"))

	# ── ⑩ ★소프트 게이트 ㉠ ────────────────────────────────────────────────
	print("── ⑩ 소프트 게이트 ㉠ ──")
	_check("⑩a 로스터 상수가 조연 T1을 담고 점주(T2)는 안 담는다(★ken 선등재)",
		m.CHORUS_GATE_ROSTER.has("mochi") and m.CHORUS_GATE_ROSTER.has("neo")
		and m.CHORUS_GATE_ROSTER.has(KID) and m.CHORUS_GATE_ROSTER.has("ken")
	...
	_drain(m)
	# 소급 적용 — 모찌·네오의 ♡3 비밀 비트도 같은 게이트를 탄다.
	_set_mains(m, 0)
	var retro_ok := true
	for sub in ["mochi", "neo"]:
		var rs: Resident = m._resident(String(sub))
		m._heart_bits.erase(String(sub))
		_set_heart(rs, 2)
		if not m._try_heart_promotion(rs).is_empty() or rs.affinity.hearts() != 2:
			retro_ok = false
	_check("⑩h ★모찌·네오의 기존 ♡3 비밀 비트에도 소급 적용된다", retro_ok)
	# 점주(T2)는 대상 밖 — 미접촉이어도 ♡3이 오른다.
	var rb: Resident = m._resident("boatman")
	_set_heart(rb, 2)
	m._try_heart_promotion(rb)
	_check("⑩i 점주 5인(T2)은 대상이 아니다(그날 밤 고백이 없는 티어)",
		rb.affinity.hearts() == 3)
	...
	if _fail == 0:
		print("══ kkaebi_arc_test 전체 통과 ══")
	else:
		print("══ kkaebi_arc_test 실패 %d건 ══" % _fail)
	quit(1 if _fail > 0 else 0)

# ── 금칙어(31어) ────────────────────────────────────────────────────────────
# 기존 아크 스위트 4종(miho·mel·bana·t1)의 목록 **합집합 23어** + [ADR-0068] 결정 6이 명문화한
# 중심 진실 4금지의 어휘 8어. 조연 파일마다 이 배열을 상속한다.
const FORBIDDEN := [
	# 플레이어 죄목의 평결(공통 18어)
...
# 깨비의 세 스테이션이 다른 주민 누구의 스케줄 칸과도 안 겹치는가.
...
# 깨비 발신 편지의 모든 줄(금칙어 스캔에 합류 — 편지도 캐릭터의 말이다).
