...
func _check_ranch_door_notice(m: Node) -> void:
	...
	for d in range(1, 400):
		if calm_day < 0 and Weather.allows_grazing(m._weather_on(d)):
			calm_day = d
		if snow_day < 0 and not Weather.allows_grazing(m._weather_on(d)):
			snow_day = d
	...
	m.clock.minutes = 10.0 * 60.0
	_check("⑨e 무대: 그 날은 방목 금지 날씨다(%s)" % Weather.NAMES[m._weather_on(snow_day)],
		not m._weather_calm())
