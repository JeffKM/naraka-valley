...
func _run_checks() -> void:
	...
	# ★[폴리시 R11 #31] 큐를 **총량이 아니라 구성으로** 잰다(s9_narrative_smoke ②c와 같은 정정).
	#   재는 것은 "미호의 그 편지가 도착했는가"인데 `pending_count() == 0`은 R8이 아침 훅 끝에
	#   조건 없이 큐잉하는 채널 개통 안내(`herald_notice` — 관문과 무관한 다른 층)까지 함께 세었다.
	...
	_check("⑤c2 큐에 남은 것은 채널 개통 안내 한 통뿐이다(R8 — 미호 편지가 아니다)",
