extends SceneTree
# ★[S9b-T2 / ADR-0068 결정 3·5·6·12] 스칼렛 풀 온보딩 — 헤드리스 검증.
#
# 규약은 kkaebi_arc_test·ken_arc_test를 그대로 상속한다(금칙어 31어 가드·볼륨 측정·훅 존재·
# 관문 드레인·절기 물음·생일·편지). 스칼렛 고유분은 ⑨의 구조 단언들이다.
#
# 무엇을 보증하나:
...
#      순간이동 위험을 구조적으로 없앤다 — 깨비 선례), 재생이 끝나면 관문 발화가 맨 앞에 선
#      대화가 열린다.
#   ⑤ 여진 편지 — ♡2·♡3·♡4가 각기 실존하는 편지를 지목하고(♡1은 없음) 발신인이 스칼렛이며,
#      관문 성사가 큐에 넣고 다음 날 아침 도착한다.
#   ⑥ 절기 물음 4개 — 짝이 맞고, 주 첫날 첫 대화에 서며, **0점 계약**(선택 전후 점수·stage 불변).
#   ⑦ 생일 — 훅 본문 + Resident.BIRTHDAYS 배정(망연절) + main 경로가 물린다.
#   ⑧ 볼륨([ADR-0068] 결정 5 상한) — 대사 100~150줄 · 컷신 4 · 절기 4 · 편지 ≤3 · **spouse 4축**.
#   ⑨ 봉인 법칙([ADR-0068] 결정 6) — 금칙어 31어 스캔 0 + 조연 경계 구조 단언(자기 앞에 앉았던
#      사람을 호명하지 않는다 · **판을 소유한 자도 실명이 없다**(T3 소관 불침범) · 그 집 안의
#      누구도 부르지 않는다 · 플레이어 죄목을 겨누지 않는다 · Affinity를 모른다).
#   ⑩ ★**소프트 게이트 ㉠**(결정 6) — 메인 3인 중 1인 이상 stage≥3이면 조연 ♡3이 성사된다.
#      ⚠️ 로스터 상수(`CHORUS_GATE_ROSTER`)에 "scarlet"을 **등재하는 것은 병렬 짝 워커의 단독
#      소유**다(결합 충돌 방지 — 로스터의 주인은 그 상수 한 곳뿐). 그래서 이 스위트는 게이트가
#      **걸린 상태·안 걸린 상태 양쪽에서 똑같이 참인 것만** 잰다: ㉠ 메인 접촉 상태에서 ♡3이
#      성사된다 ㉡ ♡1·♡2는 언제나 게이트 밖. 등재 후 "미접촉이면 대기"는 짝 워커 스위트가 잰다.
#   ⑪ 개통 계약 — confession/divorce/spouse 훅이 먼저 서 있었고, **S9b-T6이
#      main `ROMANCE_OPEN`에 스칼렛을 넣어 열었다**(콘텐츠 재작업 0).
#
# 실행: ./run_tests.sh scarlet_arc   (헤드리스는 반드시 game/에서 · 순차)

const KID := "scarlet"
const NAME_KO := "스칼렛"
const HOUSE_IDX := 1        # 주민 집 2(RESIDENT_HOUSE_RECTS[1] — 광장 바로 남쪽)
...
# 무수정으로 통과해야 하기 때문이다(선주입 = 결합 안전장치).
...
func _run_checks() -> void:
	...
	_check("①b 표시명·세이브 키·관계 트랙·선물 채널",
		...
		and r.save_key == "scarlet_affinity" and r.affinity != null and r.can_gift)
	...
	_check("①d 초상화 stem = scarlet(도트 버스트 · S9b-T9 아트 패스)", r.portrait_stem == "scarlet")
	# 집 배정 = 주민 집 2(index 1). 아침 자리가 그 집 문 바로 아래 칸이어야 동선이 성립한다.
	...
	_check("①h 낮 자리가 광장 안이다(판에서 물러선 북동 끝)",
		m.NARU_PLAZA_RECT.has_point(r.schedule[1]["tile"]))
	_check("①i 집 2는 동편 주거다(강변 2채 = 세레나 예약분을 안 쓴다)", _house_is_east(m, HOUSE_IDX))
	_check("①j 모찌·깨비·켄과 다른 집이다(1인 1채)",
	...
	_check("①k 선호 선물 테이블(러브 5 · 헤이트 2 · 전부 실존·건넬 수 있음)",
		...
		and _prefs_valid())
	_check("①l ★석화 목재 헤이트(자기가 사람에게 한 짓이 굳은 형상 — 유니버설을 덮는다)",
		GiftPrefs.tier_of(KID, ItemCatalog.PETRIFIED_WOOD) == GiftPrefs.HATE
		and GiftPrefs.universal_tier(ItemCatalog.PETRIFIED_WOOD) != GiftPrefs.HATE)
	_check("①m 명옥은 유니버설 보석(라이크)에서 러브로 올라간다(값을 아는 사람)",
		GiftPrefs.tier_of(KID, ItemCatalog.GEM_MYEONGOK) == GiftPrefs.LOVE
	...
	for t in [2, 3, 4]:
		if Mailbox.sender_of(String(who.heart_gate_letter(t))) != NAME_KO:
			sender_ok = false
	_check("⑤b 세 통 다 발신인이 스칼렛이다", sender_ok)
	...
	# ── ⑨ 봉인 법칙([ADR-0068] 결정 6) ─────────────────────────────────────
	print("── ⑨ 봉인 법칙 ──")
	# 기존 아크 스위트(미호·멜·바나·T1 2인·깨비·켄)의 금칙어 **31어를 그대로 상속**한다.
	...
	_check("⑨b ♡3은 **자기 파편**이다(자기 손이 가져간 것 + 자기 입이 넣은 문장까지)",
		g3.contains("마지막 한 푼까지 내 손이") and g3.contains("내 입으로 넣었어"))
	_check("⑨c ★자기 앞에 앉았던 사람을 호명하지 않는다(코러스 = 남의 조각을 대신 말하지 않는다)",
		g3.contains("이름은 안 할게") and not g3.contains("멜"))
	_check("⑨d ★판을 소유한 자도 실명이 없다(그 인물은 S9b-T3 소관 — 관계 암시까지만)",
		g3.contains("판을 가진 사람") and not g3.contains("미르"))
	_check("⑨e ★그 밤의 인물을 호명하지 않는다(옥자·약방·플레이어 죄목 0)",
		not g3.contains("옥자") and not g3.contains("약방"))
	_check("⑨f 불을 자기 것으로 주장하지도, 남에게 돌리지도 않는다(자기 몫까지만)",
		g3.contains("불은 내가 안 붙였어") and not g3.contains("미호") and not g3.contains("여우"))
	var g4 := _joined(who.GATE_HEART_4)
	_check("⑨g ♡4는 **자기 생전 죄까지만**(돌처럼 가둬두고 한 명에게도 안 준 죄)",
		g4.contains("돌처럼") and g4.contains("한 명한테도 안 줬어"))
	...
	_check("⑨j 석화(눈맞춤)를 메카닉으로 쓰지 않는다(백스토리 한정 — effect_fn 0 · 곱셈기 어휘 0)",
	...
	if _fail == 0:
		print("══ scarlet_arc_test 전체 통과 ══")
	else:
		print("══ scarlet_arc_test 실패 %d건 ══" % _fail)
	quit(1 if _fail > 0 else 0)

# ── 금칙어(31어) ────────────────────────────────────────────────────────────
# kkaebi_arc_test·ken_arc_test와 **같은 배열**이다(조연 파일마다 상속하는 그 목록).
const FORBIDDEN := [
	# 플레이어 죄목의 평결(공통 18어)
...
# 스칼렛의 세 스테이션이 다른 주민 누구의 스케줄 칸과도 안 겹치는가.
...
# 스칼렛 발신 편지의 모든 줄(금칙어 스캔에 합류 — 편지도 캐릭터의 말이다).
