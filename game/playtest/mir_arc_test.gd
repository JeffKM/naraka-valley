extends SceneTree
# ★[S9b-T3 / ADR-0068 결정 3·5·6·12] 미르 풀 온보딩 — 헤드리스 검증.
#
# 규약은 kkaebi_arc_test·ken_arc_test·seolhwa_arc_test·scarlet_arc_test를 그대로 상속한다
# (금칙어 31어 가드·볼륨 측정·훅 존재·관문 드레인·절기 물음·생일·편지). 미르 고유분은 ⑨의
# 구조 단언들과, ⑩의 **로스터 등재 + 미접촉 대기**다(선발 워커가 로스터 상수를 단독 소유하므로
# 그 등재의 실효까지 이 스위트가 잰다 — S9b-T2에서 설화 스위트가 맡았던 자리).
#
# 무엇을 보증하나:
...
#      선 대화가 열린다.
#   ⑤ 여진 편지 — ♡2·♡3·♡4가 각기 실존하는 편지를 지목하고(♡1은 없음) 발신인이 미르이며,
#      관문 성사가 큐에 넣고 다음 날 아침 도착한다.
#   ⑥ 절기 물음 4개 — 짝이 맞고, 주 첫날 첫 대화에 서며, **0점 계약**(선택 전후 점수·stage 불변).
#   ⑦ 생일 — 훅 본문 + Resident.BIRTHDAYS 배정(유화절) + main 경로가 물린다.
#   ⑧ 볼륨([ADR-0068] 결정 5 상한) — 대사 100~150줄 · 컷신 4 · 절기 4 · 편지 ≤3 · **spouse 4축**.
#   ⑨ 봉인 법칙([ADR-0068] 결정 6) — 금칙어 31어 스캔 0 + 조연 경계 구조 단언(판 앞에 앉았던
#      사람을 호명하지 않는다 · **내 간판이었던 아이도 실명이 없다**(scarlet.gd ⑨d의 대칭 —
#      상호 무호명) · 그 집 안의 누구도 부르지 않는다 · 불을 남에게 돌리지 않는다 · 플레이어
#      죄목을 겨누지 않는다 · 호감도를 모른다).
#   ⑩ ★**소프트 게이트 ㉠**(결정 6) — 이 스위트가 **로스터의 실효까지 잰다**: 로스터 상수가
#      "mir"·"luca"를 담고 점주 5인(T2)은 안 담는다 · ♡1·♡2는 게이트 밖 · **미접촉이면 ♡3은
#      대기**(발화 0 · 진급 0 · 비트 0 · 편지 0 · **점수 손실 0**) · 메인 1인만 ♡3이어도 열린다
#      (OR) · ♡4는 게이트 밖(생전 죄라서).
#   ⑪ 개통 계약 — confession/divorce/spouse 훅이 먼저 서 있었고, **S9b-T6이
#      main `ROMANCE_OPEN`에 미르를 넣어 열었다**(콘텐츠 재작업 0).
#
# 실행: ./run_tests.sh mir_arc   (헤드리스는 반드시 game/에서 · 순차)

const KID := "mir"
const NAME_KO := "미르"
const HOUSE_IDX := 6        # 주민 집 7(RESIDENT_HOUSE_RECTS[6] — 동편 주거 중 광장에 가장 가까운 집)
...
# ★소프트 게이트 ㉠를 연다(또는 닫는다) — 메인 3인 전원을 그 칸에 얌전히 앉힌다.
...
func _run_checks() -> void:
	...
	_set_mains(m, m.CHORUS_GATE_MAIN_STAGE)     # ★소프트 게이트 ㉠ 선주입(④·⑤가 관문을 태운다)
	...
	_check("①h 낮 자리가 광장 안이다(★가장자리가 아니라 한복판 — 마을 중심의 인물)",
		m.NARU_PLAZA_RECT.has_point(r.schedule[1]["tile"]))
	_check("①i 낮 자리가 통행 레인을 안 막는다(메인 복도 y36 · 다리 스파인 x52·53 회피)",
		int(r.schedule[1]["tile"].y) != 36
		and int(r.schedule[1]["tile"].x) != 52 and int(r.schedule[1]["tile"].x) != 53)
	_check("①j 집 7은 동편 주거다(강변 2채 = 세레나 예약분을 안 쓴다)", _house_is_east(m, HOUSE_IDX))
	_check("①k 모찌·깨비·켄·설화·스칼렛과 다른 집이다(1인 1채)",
		house_door != m.RESIDENT_HOUSE_DOORS[3]
		and house_door != m.RESIDENT_HOUSE_DOORS[5]
		and house_door != m.RESIDENT_HOUSE_DOORS[8]
		and house_door != m.RESIDENT_HOUSE_DOORS[0]
		and house_door != m.RESIDENT_HOUSE_DOORS[1])
	# 선호 선물 — 러브/헤이트가 실존 아이템이고 전부 건넬 수 있어야 한다.
	_check("①l 선호 선물 테이블(러브 5 · 헤이트 2 · 전부 실존·건넬 수 있음)",
		GiftPrefs.loves(KID).size() == 5 and GiftPrefs.hates(KID).size() == 2
		and _prefs_valid())
	_check("①m ★성장촉진 비료 헤이트(빨리 되게 하는 것 — 유니버설 디스라이크를 헤이트로 덮는다)",
		GiftPrefs.tier_of(KID, ItemCatalog.FERT_SPEED) == GiftPrefs.HATE
		and GiftPrefs.universal_tier(ItemCatalog.FERT_SPEED) == GiftPrefs.DISLIKE)
	_check("①n 보리차는 기본 잔이면서 러브다(초반부터 닿는 러브 — \"평평 ≠ 막힘\")",
		GiftPrefs.tier_of(KID, MenuCatalog.BARLEY_TEA) == GiftPrefs.LOVE
		and GiftPrefs.universal_tier(MenuCatalog.BARLEY_TEA) == GiftPrefs.LIKE)
	...
	for t in [2, 3, 4]:
		if Mailbox.sender_of(String(who.heart_gate_letter(t))) != NAME_KO:
			sender_ok = false
	_check("⑤b 세 통 다 발신인이 미르다", sender_ok)
	...
	_check("⑨b ♡3은 **자기 파편**이다(판의 소유 + 내려보낸 한 문장까지)",
		g3.contains("판을 하나 가지고 있었다") and g3.contains("그게 내 문장이다"))
	_check("⑨c ★판 앞에 앉았던 사람을 호명하지 않는다(코러스 = 남의 조각을 대신 말하지 않는다)",
		g3.contains("이름도 안 할게") and not g3.contains("멜"))
	_check("⑨d ★내 간판이었던 아이도 실명이 없다(scarlet.gd ⑨d의 대칭 — 상호 무호명)",
		g3.contains("손이 빠른 아이") and not g3.contains("스칼렛"))
	_check("⑨e ★그 밤의 인물을 호명하지 않는다(옥자·약방·플레이어 죄목 0)",
		not g3.contains("옥자") and not g3.contains("약방"))
	_check("⑨f 불을 남에게 돌리지 않는다(자기 무관여까지만 — 불씨의 주인은 안 부른다)",
		g3.contains("나는 아무것도 안 했다")
		and not g3.contains("미호") and not g3.contains("여우") and not g3.contains("도깨비"))
	_check("⑨g 결론을 대신 내리지 않는다(안다고 말하는 것 자체를 스스로 막는다)",
		g3.contains("변명이 되겠지"))
	var g4 := _joined(who.GATE_HEART_4)
	_check("⑨h ♡4는 **자기 생전 죄까지만**(용에 집착해 곁을 내친 오만·고독)",
	...
	_check("⑨k 승천·비늘의 힘을 메카닉으로 쓰지 않는다(백스토리 한정 — 곱셈기 0 · 경제 무접촉)",
		...
		and not _src().contains("margin") and not _src().contains("Cafe"))

	# ── ⑩ ★소프트 게이트 ㉠(선발 워커 판 — 로스터 등재의 실효까지) ─────────
	print("── ⑩ 소프트 게이트 ㉠ ──")
	_check("⑩a 로스터 상수가 미르를 담고 점주(T2)는 안 담는다(★luca 선등재)",
		m.CHORUS_GATE_ROSTER.has(KID) and m.CHORUS_GATE_ROSTER.has("luca")
		...
		and m.CHORUS_GATE_ROSTER.has("seolhwa") and m.CHORUS_GATE_ROSTER.has("scarlet")
	...
	if _fail == 0:
		print("══ mir_arc_test 전체 통과 ══")
	else:
		print("══ mir_arc_test 실패 %d건 ══" % _fail)
...
# 미르의 세 스테이션이 다른 주민 누구의 스케줄 칸과도 안 겹치는가.
...
# 미르 발신 편지의 모든 줄(금칙어 스캔에 합류 — 편지도 캐릭터의 말이다).
