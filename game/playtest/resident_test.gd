...
func _run_checks() -> void:
	...
	# ★[S9b-T6 / ADR-0068 결정 3·4] 19 → 20: 세레나(조연 코러스 **마지막** · [narrative-bible §5.3]이
	...
	# ★[S9b-T6 / ADR-0068 결정 3] 17 → 18행: 세레나가 강림 뒤에 붙었다(같은 등록 순서 규약 ·
	#   조연 코러스의 마지막 행). ⚠️ 위 경고를 그대로 이행한 자리다 — ③a·③b와 **한 커밋에서
	#   함께** 고쳤다(S9b-T3이 여기를 빠뜨려 결합 회귀가 잡아낸 그 전례를 안 되풀이한다).
	_check("⑧a 관계 트랙 보유 18인 전원(옥자·주방요괴만 제외)",
		names == ["미호", "멜", "바나", "네오", "모찌", "뱃사공", "옹이", "풀무", "무골",
			"깨비", "켄", "설화", "스칼렛", "미르", "루카", "프로스티", "강림", "세레나"])
