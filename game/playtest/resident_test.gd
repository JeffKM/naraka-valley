...
func _run_checks() -> void:
	...
	# ★[S9b-T4 / ADR-0068 결정 3] 17 → 18: 프로스티(조연 코러스 7번째 · 비인간 3인의 마지막)가
	#   **미르·루카 뒤, 주방요괴 앞**에 붙었다(의도적 불변식 개정). 단독 태스크라 이번엔 한 명이다.
	# ★[S9b-T5 / ADR-0068 결정 3·4] 18 → 19: 강림(조연 코러스 8번째 · 로스터 최고 수위라 결정 4가
	#   단독 태스크로 못 박은 자리)이 **프로스티 뒤, 주방요괴 앞**에 붙었다(의도적 불변식 개정).
	_check("③a 주민 19인 등록(★T8 모찌 · ★S3-T5 뱃사공 · ★S4-T7 옹이 · ★S5-T3 풀무 · ★S5-T6 무골 · ★S9b-T1 깨비·켄 · ★S9b-T2 설화·스칼렛 · ★S9b-T3 미르·루카 · ★S9b-T4 프로스티 · ★S9b-T5 강림 · ★S6-T7 주방요괴 추가)",
		m._residents.size() == 19)
	_check("③b 19인 = 미호·멜·바나·네오·옥자·모찌·뱃사공·옹이·풀무·무골·깨비·켄·설화·스칼렛·미르·루카·프로스티·강림·주방요괴",
	...
	# ★[S9b-T2 / ADR-0068 결정 3] 11 → 13행: 설화·스칼렛이 켄 뒤에 붙었다(같은 등록 순서 규약).
	# ★[S9b-T3 / ADR-0068 결정 3] 13 → 15행: 미르·루카가 스칼렛 뒤에 붙었다(같은 등록 순서 규약).
	# ★[S9b-T4 / ADR-0068 결정 3] 15 → 16행: 프로스티가 루카 뒤에 붙었다(같은 등록 순서 규약).
	#   ⚠️ 이 단언은 총원 단일 출처(③a·③b)의 **파생**인데 자동으로 따라오지 않는다 — 조연을
	#   더하는 태스크는 ③a·③b와 이 ⑧a를 **함께** 고쳐야 한다(S9b-T3이 여기를 빠뜨려 결합 회귀가
	#   잡아낸 전례가 있다).
	# ★[S9b-T5 / ADR-0068 결정 3] 16 → 17행: 강림이 프로스티 뒤에 붙었다(같은 등록 순서 규약).
	_check("⑧a 관계 트랙 보유 17인 전원(옥자·주방요괴만 제외)",
		names == ["미호", "멜", "바나", "네오", "모찌", "뱃사공", "옹이", "풀무", "무골",
			"깨비", "켄", "설화", "스칼렛", "미르", "루카", "프로스티", "강림"])
	# ★[S8-T9] 선물 리듬 꼬리("이번 주 선물 n/2")가 붙어 곱셈기 없는 주민도 줄이 선다 — 곱셈기
	#   유무는 꼬리 **앞**에 무엇이 있느냐로 읽는다(frame_test ③b′와 같은 기준).
	# ★[S9b-T1·T2] 깨비·켄·설화·스칼렛(꼬리 4행)도 곱셈기 없는 쪽이다 — 조연 = 쉼터 2채널(ADR-0008).
