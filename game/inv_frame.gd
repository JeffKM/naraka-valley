...
# ★ 키는 main._heart_badge()가 만드는 **그 문자열 그대로**다(안정 UI 문자열 — 못 찾은 값은 금박으로
#   떨어져 새 배지가 생겨도 안 깨진다).
# ★[폴리시 R30 #21] **「서운함」이 다섯째 값이다.** 옛 머리말은 로스터를 넷으로 못 박았는데 R21 #23이
#   질투 감점 배지를 그 뒤에 하나 더 낳았고, 표에 없으니 폴백 금박으로 떨어졌다 — 금박은 이 UI에서
#   「지금 할 수 있는 일이 있다」를 뜻하도록 설계된 색이라(진급 대기·숙련 배지·마일스톤 claimable이
#   전부 금박이다) **정반대 의미의 페널티 상태가 같은 픽셀로 섰다.** 그 색의 뜻을 말하는 범례가
#   이 탭에 0이므로 색이 곧 뜻이고, 그래서 색이 뜻을 뒤집었다. 식은 잿빛-청을 준다: 관계 삼색
#   (진홍 계열)과도, 금박(행동 가능)과도 겹치지 않는 «지나가는 상태»의 톤이다.
const HEART_BADGE_COLORS := {
	...
	"서운함": Color(0.58, 0.62, 0.68),          # 식은 잿빛-청(질투 감점 = 페널티·행동 불요)
}
