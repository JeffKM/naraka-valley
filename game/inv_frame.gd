...
# ★ Phase B 옵션 탭 — 액션만(저장 · 저장하고 나가기). 설정 화면 본체(볼륨·전체화면)는 Phase D.
func _draw_options_tab(panel: Rect2, font: Font) -> void:
	var x := panel.position.x + PAD + 12.0
	var y := panel.position.y + PAD + 48.0
	_save_rect = Rect2(x, y, 200.0, 30.0)
	draw_rect(_save_rect, Color(0.20, 0.24, 0.16, 0.85))
	draw_rect(_save_rect, Color(0.55, 0.60, 0.40), false, 1.0)
	draw_string(font, _save_rect.position + Vector2(14.0, 20.0), "저장", HORIZONTAL_ALIGNMENT_LEFT, -1, 15, Color(1, 0.97, 0.88))
	_quit_rect = Rect2(x, y + 42.0, 200.0, 30.0)
	draw_rect(_quit_rect, Color(0.28, 0.16, 0.14, 0.85))
	draw_rect(_quit_rect, Color(0.62, 0.42, 0.36), false, 1.0)
	draw_string(font, _quit_rect.position + Vector2(14.0, 20.0), "저장하고 나가기", HORIZONTAL_ALIGNMENT_LEFT, -1, 15, Color(1, 0.94, 0.86))
	draw_string(font, Vector2(x, y + 92.0), "볼륨·전체화면 설정은 준비 중", HORIZONTAL_ALIGNMENT_LEFT, -1, 12, Color(0.72, 0.66, 0.56))
...
func _click_menu(p: Vector2) -> void:
	...
	for i in _tab_rects.size():
		if _tab_rects[i].has_point(p):
			set_tab(i)
			return
	# ★ Phase B 옵션 탭: 저장 / 저장하고 나가기(main이 실제 저장·종료 수행 — 프레임은 신호만).
	if menu_tab == TAB_OPTIONS:
		...
		return
	if menu_tab != TAB_INV:
		return   # 관계·숙련 탭은 읽기 전용(본문 클릭 없음)
	# 정리 버튼.
