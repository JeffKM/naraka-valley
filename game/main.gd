...
func _build_home() -> void:
	...
	_set_tile(STOREHOUSE_DOOR_E.x, STOREHOUSE_DOOR_E.y, HOUSE)  # ★[ADR-0046] 실내 창고 문 동칸 개방(2칸·중앙 — 실내문≡외관문)
	# ★ [B1-a.1] 동물 2건물 실내 방(빈 그레이박스 방 — 짐승은 _draw_ranch가 그림, 여물통=B1-a.3). 실내문≡외관문 2칸.
	_build_room(NEOKURITGAN_RECT, HOUSE, HOUSE_WALL, NEOKURITGAN_DOOR)  # 넋우릿간 실내
	_set_tile(NEOKURITGAN_DOOR_E.x, NEOKURITGAN_DOOR_E.y, HOUSE)
	_build_room(NEOKDUNGURI_RECT, HOUSE, HOUSE_WALL, NEOKDUNGURI_DOOR)  # 넋둥우리 실내
	_set_tile(NEOKDUNGURI_DOOR_E.x, NEOKDUNGURI_DOOR_E.y, HOUSE)
...
# 옛 1타일 절벽(x23열·y29행·CLIFF_CORNER 타일종)은 폐기·상수 제거 완료(cleanup 커밋) — 코너는 _lay_corner_step(FACE).
...
# ★ [B1-a.1] 신규 게임 스타터 짐승 배치. ranch가 비었을 때만(멱등) 종별 소속 건물 실내에 1마리씩 놓는다
# (진입 실내 — 안개소=넋우릿간·노을닭=넋둥우리). 방목 왕래(B1-a.2) 전이라 실내 거주가 초기 상태다.
...
func _ensure_starter_animals() -> void:
	if ranch == null or ranch.count() > 0:
		return
	_seed_starter_animal(AnimalCatalog.HONBAEK_SO, "넋우릿간", NEOKURITGAN_RECT)   # 안개소(대형·barn형)
	_seed_starter_animal(AnimalCatalog.HONBAEK_DAK, "넋둥우리", NEOKDUNGURI_RECT)  # 노을닭(소형·coop형)

# 건물 실내 바닥(벽 1칸 안쪽) 첫 빈 칸에 종을 소속 건물째 배치. 방 rect는 둘레가 벽이라 안쪽으로 파고든다.
func _seed_starter_animal(species: String, building: String, room: Rect2i) -> void:
