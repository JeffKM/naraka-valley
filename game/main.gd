...
const LANTERN_TILES_HOME := [Vector2i(38, 48), Vector2i(42, 48)]  # 안식 농원 농장 입구(외부) ★T3②: 밭 남단 스파인(x40) 양옆 등불
...
const PROP_LAYOUT_HOME := [
	...
	[PROP_LANTERN, LANTERN_TILES_HOME],                                                  # 안식 농원 농장 입구 등불 둘(외부)
	[PROP_POT, [Vector2i(18, 68)], WALL_PROP_LIFT],                                      # 집 우상단 화분(상단 벽 flush)
	[PROP_POT, [Vector2i(9, 74), Vector2i(18, 74)]],                                     # 집 좌하·우하 화분(하단 벽 — lift 없음)
	# ── ★ T3② 외부 농장 장식(밭 x26..53·y10..47 — 작물·중앙 길 x40·외관·문 비껴, 충돌 0) ──
	# 밭 남단 가로 울타리(y48 GROUND — 좌우 가장자리까지 닿는 연속 런, 스파인 x40 게이트만 비움).
	# ★ farm_fence = 가로 연결 전용(좌우 레일이 edge까지 닿아 칸끼리 이어짐) — 세로 배치 안 함(연결 안 됨).
	[PROP_FENCE, [
		Vector2i(26, 48), Vector2i(27, 48), Vector2i(28, 48), Vector2i(29, 48), Vector2i(30, 48),
		Vector2i(31, 48), Vector2i(32, 48), Vector2i(33, 48), Vector2i(34, 48), Vector2i(35, 48),
		Vector2i(36, 48), Vector2i(37, 48), Vector2i(38, 48),
		Vector2i(42, 48), Vector2i(43, 48), Vector2i(44, 48), Vector2i(45, 48), Vector2i(46, 48),
		Vector2i(47, 48), Vector2i(48, 48), Vector2i(49, 48), Vector2i(50, 48), Vector2i(51, 48),
		Vector2i(52, 48), Vector2i(53, 48),
	]],
	[PROP_SCARECROW, [Vector2i(24, 12), Vector2i(55, 44)]],                              # 허수아비 둘(밭 서북·동남 곁 GROUND)
	[PROP_PLANTER, [Vector2i(38, 52), Vector2i(42, 52), Vector2i(38, 57), Vector2i(42, 57)]],  # 농장 입구~스폰 접근로 화분(GROUND, 밭·길 비껴)
	[PROP_FLOWER_PATCH, [
		Vector2i(10, 20), Vector2i(70, 20), Vector2i(8, 42), Vector2i(72, 42), Vector2i(18, 56), Vector2i(62, 56),
		Vector2i(6, 12), Vector2i(74, 14), Vector2i(16, 32), Vector2i(64, 36), Vector2i(20, 44), Vector2i(60, 24),
	]],  # 꽃 패치 산재(서/동/남 코지 여백 — 휑함 완화, 클러터 X)
	...
	[PROP_GRASS, [Vector2i(6, 16), Vector2i(8, 40), Vector2i(4, 50), Vector2i(72, 20),
		Vector2i(74, 36), Vector2i(70, 52), Vector2i(16, 6), Vector2i(60, 6),
		Vector2i(22, 60), Vector2i(58, 60)]],                    # 풀 무더기 산재(장식)
	[PROP_BUSH, [Vector2i(4, 38), Vector2i(72, 48), Vector2i(64, 8)]],  # 덤불(장식)
	[PROP_STUMP, [Vector2i(14, 6), Vector2i(66, 30), Vector2i(10, 60)]],  # 그루터기·통나무(장식)
]
...
const HOUSE_EXT_RECT := Rect2i(36, 2, 9, 8)    # x36..44, y2..9 (★T3② 북중앙 농가 — 밭 바로 위, 9×8로 확대 ★①)
...
const HOUSE_EXT_DOOR := Vector2i(40, 9)    # 외관 집 문(닿으면 진입) — 아래벽 중앙, 남쪽 밭으로 바로 열림(현관 앞 농장 ★②)
...
# ── ★ T3② — 80×65 안식 농원 '현관 앞 농장' 재배치(스타듀식 농가-북/밭-남) ────────────
# 집(북중앙 x36..44) / 창고(서북 x8..13) / 축사(동북 x66..71) = 상단 농가 군집, 밭은 집 문(40,9)
# 바로 남쪽(y10~)으로 붙어 "문 열면 바로 밭". 스폰(남단)에서 중앙 스파인(x40)을 따라 밭을 가로질러
# 집까지 걷는 동선. GROUND/SOIL이 열려 도달성은 자동(길은 시각 레인) — _carve_paths가 안내 동선을 깐다.
const FARM_RECT := Rect2i(26, 10, 28, 38)  # x26..53, y10..47 (집 문 바로 남쪽 ~1064타일, 현관 앞 농장)
...
const STOREHOUSE_EXT_RECT := Rect2i(8, 3, 6, 6)    # ★T3② x8..13, y3..8 (서북 부속 — 밭 x26~·집 x36~ 비껴)
const STOREHOUSE_EXT_DOOR := Vector2i(11, 8)       # 외관 창고 문(닿으면 진입) — 아래벽 중앙, _carve_paths 서편 레인 연결
...
const BARN_EXT_RECT := Rect2i(66, 4, 6, 4)     # ★T3②: x66..71, y4..7 (동북 부속 — 밭 x≤53·동워프 y32 비껴)
const BARN_EXT_DOOR := Vector2i(68, 7)         # 축사 문 리세스(아래벽 중앙, 시각 일관용 — 진입 트리거 아님, 카탈로그 미등록)
...
func _carve_paths() -> void:
	# ★ T3② — 80×65 '현관 앞 농장' 동선(시각 레인). GROUND/SOIL이 열려 도달성은 자동이라 길은 *안내*다.
	#   중앙 스파인(x40)이 집 문(40,9)에서 밭을 가로질러 스폰(40,60)까지 내리꽂혀 "내 농장 한복판을
	#   걸어 현관으로". 남단 가로 복도(y50)가 서편(창고)·동편(축사) 레인을 잇고, 동워프(78,32)는
	#   동편 레인에서 y32로 갈라져 닿는다(스타듀식 가장자리 워프).
	_carve_v(40, 9, 60)                         # 중앙 스파인: 집 문(40,9) → 밭 가운데 → 스폰(40,60)
	_carve_h(50, 11, 68)                        # 남단 가로 복도(서 창고레인 x11 ~ 동 축사레인 x68)
	_carve_v(11, 8, 50)                         # 창고 문(11,8) → 남단 복도 — 서편 레인
	_carve_v(68, 7, 50)                         # 축사 앞(68,7) → 남단 복도 — 동편 레인
	_carve_h(32, 68, 78)                        # 동편 레인(x68)서 갈라져 동쪽 길 워프(78,32)
