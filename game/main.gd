...
const SOLID_TILES := [HOUSE, CAFE, WALL, HOUSE_WALL, CAFE_WALL, WATER, TREE]   # 아틀라스 가로 배치 순서(= atlas x)
...
# 무대 + 채집지(라벨만) + 목공방(enterable 빈 방)까지. 나루 마을 산길(38,8) 임시 직결로 남단 spawn
# (20,22)에 도착(M4.1 도달성 — 정규 갱도 경로는 갱도 빌드 시). 동단(38,16)이 미혹의 숲으로 가는 워프(M4.2 점등).
# 동선 = 가로 복도(y16, 서 우회 1,16 ~ 동 미혹 38,16) + 남단 세로(spawn 20,22·갱도 워프 20,23) + 목공방 문.
...
func _build_tileset() -> TileSet:
	...
	for solid in [WALL, HOUSE_WALL, CAFE_WALL, WATER, TREE]:
...
func _carve_jeoseung_forest_paths() -> void:
	_carve_h(16, 1, 38)                    # 가로 복도(서 우회 1,16 ~ 동 미혹 워프 38,16)
	_carve_v(20, 16, 23)                   # 남단 spawn(20,22)·갱도 워프(20,23) → 복도
...
func _place_labels() -> void:
	...
	match _region:
		...
		RegionCatalog.NARU_VILLAGE:
			...
			_add_label("산길 → 저승 숲", _tile_center_px(Vector2i(36, 10)))   # ★ M4.1 동 산길(임시 직결, 갱도 빌드 시 갱도로 원복)
		RegionCatalog.SAMDOCHEON:
		...
		RegionCatalog.JEOSEUNG_FOREST:
			...
			_add_label("← 나루 마을", _tile_center_px(Vector2i(4, 15)))         # ★ M4.1 서단 임시 우회 워프(1,16)
			_add_label("갱도 →(준비 중)", _tile_center_px(Vector2i(20, 21)))   # 남단 숲길 워프(20,23, 휴면 — 갱도 빌드 시 점등)
		RegionCatalog.MIHOK_FOREST:
