...
# ★ ADR-0035 안식 재설계 Phase B — 절벽 단면(통과 X). 높이=2D평면+절벽 충돌+계단(통과)이라 z축 아님.
# 남향(south-facing) 절벽면이 고지(하늘 목장 NW)의 남쪽 가장자리를 두르고, 계단으로만 진입한다.
# 32px 도트(Phase A 마스터 스타일) — WALL과 같은 결로 SOLID source에 들어가 통과 불가 충돌을 받는다.
# 코너 틈은 넝쿨(vine)·덤불(bush) Y-Sort 덮개로 가린다(타일 재도트 없이, owner 결정).
const CLIFF_FACE := 12      # 남향 절벽면(E-W 단면)
...
const SOLID_TEX := {
	...
	CLIFF_CORNER_L: "res://assets/tiles/cliff_corner_l.png",
	CLIFF_CORNER_R: "res://assets/tiles/cliff_corner_r.png",
	CLIFF_INNER: "res://assets/tiles/cliff_inner.png",
}
...
const HOUSE_EXT_RECT := Rect2i(40, 2, 9, 8)    # ★ADR-0035 Phase B x40..48, y2..9 (북중앙 본가 — 저지, 창고 오른쪽 병렬)
...
const HOUSE_EXT_DOOR := Vector2i(44, 9)    # ★ADR-0035 외관 본가 문(닿으면 진입) — 아래벽 중앙(x40..48), 남쪽 스타터 패치로 열림
...
func _place_labels() -> void:
	...
	match _region:
		RegionCatalog.HOME:
			...
			_add_label("하늘 목장(개간 예정)", _tile_center_px(Vector2i(12, 14)))  # ★ADR-0035 고지(절벽+계단 너머, 목축 Slice 1)
