...
#   - **★[S4-T8] 황천해 백사장 존 배선 완료.** T1이 "아이템만 등록·존 배선은 T8"로 남겨 둔 자리를
#     채웠다. 문법은 **한 줄도 새로 안 만들었다** — 같은 zones() 테이블·같은 상한 6·같은 7일 리셋·
#     같은 결정 롤을 그대로 탄다(ADR-0062 결정 9 ㉢ "기존 ForageSpawns 문법 재사용"이 이 항목의
#     목적 자체다). 갈리는 건 종 풀이 절기 무관이라는 것뿐이고, 그건 이미 species_for가 든다.
...
static func zones() -> Dictionary:
	return {
		...
		# ★[S4-T8 / ADR-0062 결정 9 ㉢] 황천해 백사장 존 — 무대 재사용(신규 맵 0). rect는 모래 밴드
		#   (HWANG_BEACH_RECT y19~27) 안에서 **동편 빈 모래**를 골랐다: 생선가게 외관(x8~14)·부두
		#   (x28)·나룻배 프롭(x34~35)·산책로(y26)를 전부 비껴가고, 남쪽 바다(y28~)와도 세 칸 떨어져
		#   있다. 즉 존 35칸이 전부 걸을 수 있는 모래다(forage_extras_test ③이 실그리드로 단언).
		RegionCatalog.HWANGCHEONHAE: [
			{"rect": Rect2i(40, 20, 7, 5), "kind": KIND_BEACH},    # 백사장 동편(x40..46, y20..24)
		],
	}
