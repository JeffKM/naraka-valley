...
# ★[S9b-T6 / ADR-0068 결정 2 · narrative-bible §5.3] 뭍의 비약 — **세레나 결혼 전용 추가 전제**.
#   §5.3이 "결혼은 옥자 비약 퀘스트로 다리(마녀 서비스 한정·옥자 게이트 불가침)"라 못 박은 그
#   비약이다. 물 밖에 못 나오는 인어라 혼례·이주가 성립하려면 뭍에서 견디는 몸이 먼저 필요하고,
#   그것을 **옥자가 마녀 서비스로** 지어 준다(혼례 부적과 정확히 같은 창구·같은 문법 — 새 동선 0).
#   ★ id에 okja가 남는 것은 바이블 자구(「옥자 비약」)의 계보이고, 표시 이름은 CONTEXT 명명 결을
#     따라 **쓰임**으로 짓는다(부적이 「혼례 부적」인 것과 같은 규약 — 만든 이가 아니라 하는 일).
#   ⚠️ **옥자 트랙(호감도·deed)과 무관하다** — 옥자는 관계 트랙이 없는 서사 앵커이고([ADR-0068]
#     결정 10의 deed 채널은 B6 후에나 열린다), 이 비약은 그 트랙을 한 칸도 안 건드리는 *서비스*다.
#   KEYS인 이유 = 혼례 부적과 같은 결(비매·유니크·스택 불가·기증 불가)이라 새 dict가 필요 없다.
const OKJA_ELIXIR := "okja_elixir"          # 뭍의 비약 — 세레나 청혼의 추가 전제(옥자 의뢰)
# ★[S9b-T8 / ADR-0068 결정 10 · narrative-bible §6.3] 명부 혼례 부적 — **궁극의 결혼 분기 전용
#   정표 특별판**. 보통 부적은 옥자가 접는 마녀 서비스인데, 옥자 본인의 혼례만은 자기가 자기 것을
#   접을 수 없다. §6.3이 "옥자 본인 결혼은 강림이 맺어 준다"고 못 박았으므로 **명부의 주인이
#   접는다** — 계약으로 묶던 손이 사랑으로 묶는 거울이고, 그 사람이 곧 주례다(B7 연출).
#   ★ **값이 없다**(냥 0): 부적 5,000·비약 3,000과 갈리는 자리다. 명부에 이름을 올리는 일은
...
#   · **원목·석화 목재·광물은 여기 안 든다** — 제작(`CraftCatalog`)·건축(`Carpenter`)·제련
#     (`Furnace`)이 실제로 삼키므로 창구가 0이 아니고, 자재군 전체를 여는 것도 아니다
#     (출하 술어는 이 표에 실린 id만 받는다).
#   · 나락혼정도 안 든다 — 카페 시그니처(곳간 → 서빙)가 소비처다.
# ★[폴리시 R29 #13] **위 배제 문장이 여덟에 대해 사실이 아니었다.** R13이 「단단한 원목·수액·
#   이끼는 제작·건축·제련이 이미 삼킨다」고 적었는데 전수 대조가 그 셋을 한 곳도 못 찾는다:
#   제작 15레시피의 mats·건축(gold + `wood` + `ingot`뿐)·제련(광석 + 혼탄)·게시판 의뢰 풀
#   (`CropCatalog.ids() + FORAGEABLES`)·시련 납품 풀(MINERALS 한정)·카페 시그니처(FUSIONS)
#   어디에도 없다. 등재 조건 ㉠㉡을 그대로 다시 재면 **여덟이 통과한다**(가격은 이 표가 이미
#   선언해 둔 값 그대로 — 새 수치 0):
#     · 혼백 섬유(4) — 재점령 잡초를 벨 때마다 1개씩. 매일 쌓이는데 처분이 휴지통뿐이었다.
#     · 업화석 조각(12) — 개간·절기 재스폰 debris의 곡괭이 산출.
#     · 수액(3) — 벌목마다 5개씩. 위와 같은 이유로 백팩 칸만 먹었다.
#     · 저승 이끼(8) — 낫 채취 상시 자원.
#     · 단단한 원목(35) — **가장 무거운 자리**다. ADR-0052 2단 전문직 「벌목꾼」(채집 Lv10·
#       비가역)의 **유일한 퍼크 산출물**이자 채집 경지 유물의 답례(`Mastery` 50개)인데 sink가
#       0이라, 형제 후보(수액꾼 — 산출이 CAT_HARVEST라 정상 판매)와의 비대칭이 곧 «한쪽을
#       고르면 그 선택이 무효»였다(R29 #14가 그 귀결을 따로 짚었다 — 여기서 함께 닫힌다).
#     · 나무 씨앗 3종(각 12) — 벌목 부산물. 심는 창구가 없다(자체 파종은 원장이 스스로 굴린다).
#   ★ 선물은 sink로 세지 않는다 — 이미 실린 셋(넋가루·혼불씨)도 선물 러브 칸을 갖는다(그 판정은
#     캐릭터별 취향이지 «값을 받는 창구»가 아니다). 등재 조건은 위 ㉠㉡ 그대로다.
...
const SHIPPABLE_MATERIALS := {
	...
	NEOKGARU: true,
	HONBULSSI: true,
	# ★[폴리시 R29 #13·#14] 위 머리말의 여덟(개간·벌목·채취의 상시 산출 + 벌목꾼 퍼크 산출물).
	SOUL_FIBER: true,
	EMBER_SHARD: true,
	SAP: true,
	JEOSEUNG_IKKI: true,
	HARDWOOD: true,
	SEED_JEOSEUNGSOL: true,
	SEED_MYEONGDANPUNG: true,
	SEED_NEOKCHAM: true,
}
