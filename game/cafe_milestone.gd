...
#
# ★[폴리시 R3 — 진행 교착 봉합] **하트 문턱 9 → 8**(= `2 × Affinity.GATE_MAX`, 파생).
#   위 "최대 10 중 9"라는 셈은 S8-T5가 **진급 관문**(main `HEART_GATE_MAX` = `Affinity.GATE_MAX` = 4)을
#   넣은 뒤로 성립하지 않는다. 관문으로 오를 수 있는 칸은 ♡4까지이고 ♡5는 *의지 시험*(고백)만
#   여는데, 연애 슬롯은 **전 로스터 공유 1개**다(main `_romance_partner`). 그래서 미호+멜 합의
#   실제 상한은 10이 아니라
#     · 미호나 멜과 연애하면 5+4 = **9**
#     · 그 밖(바나·조연 11인·옥자)과 연애하거나 아무와도 연애하지 않으면 4+4 = **8**
#   이다. 문턱이 9면 **연애 상대 선택 하나가 카페 2·3단을 영구 잠그는** 판이 생긴다(좌석 5·곳간
#   50·융합 슬롯·손님 볼륨·바캉스 데이·늘봄방이 전부 이 술어 뒤에 있다). 관계를 *메카닉 게이트*로
#   만들지 않는다는 ADR-0008("평평 ≠ 막힘")의 정면 위반이라, **연애 선택과 무관하게 닿는 상한**인
#   `2 × GATE_MAX`로 내린다. 이제 문턱은 상한에서 파생되므로 관문 상한을 조정해도 다시 어긋나지 않는다.
#   ⚠️ owner 큐(눈금): 이 값은 "관문만으로 닿는 최대"라 **여지가 0칸**이다. 선언부가 원래 노렸던
#     "한 칸의 여지"까지 원하면 7이고, 그 선택은 21일 곡선 결재 사안이라 여기서 정하지 않았다.
const S2_TARGET_HARVEST := 40
const S2_TARGET_REVENUE := 1800
const S2_TARGET_HEARTS := 2 * Affinity.GATE_MAX
