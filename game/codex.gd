...
#
# 등재 기준 = **멜 출하대 출하 1회 이상**. 이 한 문장이 이 파일의 전부다.
#   ㉠ 선물·요리 재료 사용·상자 보관은 **미집계**다. 억지로 배제한 게 아니라, [ADR-0021]이 판매를
#      출하대 단일 창구로 못 박아 둔 덕에 "판매 이력"이라는 축이 코드에 이미 하나뿐이기 때문이다.
...
static func category_ids(cat: String) -> Array:
	match cat:
		...
		CAT_COOK:
			# ★ 융합 메뉴 **만**이다 — MenuCatalog.ids()는 기본 메뉴 4종(라떼·아메리카노…)을 함께
			#   물고 있는데, 그건 카페가 원래 파는 물건이라 "모으는" 축이 아니다(수집물 = 융합 메뉴).
			return MenuCatalog.fusion_ids()
	return []
