...
# ★[S5-T6] 빈 슬롯이 하나라도 있는가. **1회성 보상(갱도 상자)의 선검사** 전용 창구다 — 되돌릴 수
# 없는 사건 전에 "받을 자리가 있나"를 먼저 묻기 위한 것이라, 스택 합치기까지 보는 정밀 판정이 아니다
# (스택 가능한 물건은 자리가 없어도 들어갈 수 있으므로 이 판정은 **보수적**이다 = 틀리면 늘 안전한 쪽).
