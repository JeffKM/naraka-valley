...
# ── 곡선 스케일(T7.3 — 슬라이스 길이에 비례) ────────────────────────────────
# T4.3 밸런싱: 14일 슬라이스에서 대화만(8점/일)으로도 ♡1≈D5·♡2≈D10이 성립하게
# 하트당 40점으로 맞췄다(miho-heart-arc 목표 곡선). 그 곡선이 14일 기준이었다.
#
# ★ T7.3 위험 B(♡5 조기 만렙) 재조정: 14일 곡선 그대로 21일을 돌리면 Gifter가 ♡5
#   만렙에 D12쯤 닿아 슬라이스 후반(D12~21)에 관계 동기가 죽는다. 그래서 하트당 점수를
#   *슬라이스 길이에 비례*해 스트레치한다 — 곡선의 모양(대화:선물 비중·여우불 깸 시점의
#   상대 위치)은 보존하고 가로축만 RUN_DAYS에 맞춰 늘린다. 21일이면 40 × 21/14 = 60점/칸.
#   ♡5 만렙이 슬라이스 후반으로 밀려 후반 동기가 유지된다.
#
# ★ 단일 진실원 = RunSummary.RUN_DAYS(손절 사다리 ①): 길이를 21↔14로 바꾸는 그 한 줄이
#   곡선까지 함께 되돌린다(14면 60 → 40으로 정확히 복귀, 40×14/14=40). 미호·멜·바나가
#   같은 affinity.gd 곡선을 공유하므로 세 호감도 모두 일관되게 따라온다.
const BASELINE_DAYS := 14                 # 곡선이 처음 튜닝된 기준 길이(♡1≈D5·♡2≈D10 앵커)
const BASELINE_POINTS_PER_HEART := 40     # 그 기준 길이에서의 하트당 점수
const POINTS_PER_HEART := BASELINE_POINTS_PER_HEART * RunSummary.RUN_DAYS / BASELINE_DAYS
