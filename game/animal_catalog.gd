...
#
# 설계 메모(§8.4):
#   - ★ 그레이박스 스코프 = 아키타입 2종(coop·barn 각 1) — 혼백 닭(닭장 결)·혼백 소(축사 결).
...
# ── 짐승 종 식별자(영문 id) ───────────────────────────────────────────────────
const HONBAEK_DAK := "honbaek_dak"   # 혼백 닭(魂魄鷄) — 닭장(coop) 결. 산물 = 혼백란(알).
const HONBAEK_SO := "honbaek_so"     # 혼백 소(魂魄牛) — 축사(barn) 결. 산물 = 혼백유(젖).

# ── 산물 아이템 id(영문 id) — 산물 = CAT_HARVEST 아이템 ────────────────────────
const HONBAEK_RAN := "honbaek_ran"   # 혼백란(魂魄卵) — 혼백 닭의 알.
const HONBAEK_YU := "honbaek_yu"     # 혼백유(魂魄乳) — 혼백 소의 젖.
