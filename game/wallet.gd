...
#     화폐만 그레이박스로 둔다.
#   - ★[폴리시 R13 / 잠정 — owner 확정 대기] **표시 단위는 "냥"으로 통일했다.** 이 머리말이
#     "공식 화폐명이 정해지기 전까지 표시는 골드"라 적어 둔 사이에 표시 층이 두 이름으로 갈렸고,
#     한 함수 안에서 실패는 "골드 부족"·성공은 "−120냥"이라 부르는 데까지 갔다(순수 결함). 갈림을
#     닫을 기준은 CONTEXT뿐이라 거기를 따랐다 — [시련패] 표제어가 "냥과 교환되지 않고"로 냥을
#     화폐로 못 박고, 인게임 시련패 문구도 같은 단어를 쓴다. **공식 명칭 확정은 여전히 owner
#     몫**이고, 바뀌면 표시 문자열만 다시 훑으면 된다: 내부 식별자(`gold`·`earn`·`spend`·세이브
#     키)와 주석은 그대로 두었으므로 로직·세이브 호환은 이 결정과 무관하다.
