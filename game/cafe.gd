...
#   - ★ seam 2 (서빙 수익 = 멜 마진): 서빙가는 BASE_PRICE × margin 한 값으로 나온다.
#     지금 margin=1.0(base rate, ♡0에서도 카페는 굴러감 — ADR-0008 "평평 ≠ 막힘").
#     T5.5가 cafe_margin.gd로 margin(♡)를 얹어 ×1.0→×2.0으로 분화한다(관계=곱셈기).
...
# ★seam 2: 서빙 단가 배수. base rate=1.0(ADR-0008). T5.5가 margin(♡)로 갈아끼운다.
...
# ★seam 2: 서빙 단가 = 정액 P × margin 한 값. 지금 margin=1.0이라 P_base 그대로.
