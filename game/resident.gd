...
const BIRTHDAYS := {
	...
	"serena": [0, 26],    # ★[S9b-T6] 세레나 = 인어. 피안절은 **언 물이 풀려 물길이 다시 흐르고
	                      #   떠났던 것이 돌아오는** 절기 — 물길에 휩쓸려 *온* 존재의 계절이
	                      #   그대로 날짜가 된다. 26일 = 절기의 끝자락을 고른 것은, 이 인물의
	                      #   반전 축이 "평생 누워만 있던 그녀가 처음 움직임"이라
	                      #   ([narrative-bible §5.3]) **풀리는 절기가 다 풀린 뒤**의 자리가
	                      #   맞기 때문이다(시작이 아니라 *마침내*의 결). 회피일(25일·피안 행사
	                      #   12일)과 기존 피안절 배정(옹이 4·뱃사공 11·네오 19·켄 23)을 전부
	                      #   비껴간다.
	# 유화절(여름결) — 불볕과 결실의 절기.
	...
	"gangrim": [2, 4],    # ★[S9b-T5] 강림 = 폐직 저승사자. 망연절은 **거두고 셈하는** 절기 —
	                      #   명부를 셈하던 자의 계절이 그대로 날짜가 된다. 나흗날을 고른 것은
	                      #   **넉 사(四)가 죽을 사(死)와 소리가 같아 다들 비껴 두는 날**이기
	                      #   때문이다: 아무도 안 고르는 그 날이 곧 죽음을 인도하던 자의 날이고,
	                      #   "축하받을 자격이 없다고 스스로 판정한 자"에게 남들이 비워 둔 날짜가
	                      #   배정되는 어긋남이 이 인물이다([narrative-bible §5.2]). 회피일
	                      #   (25일·망연 행사 16일)과 기존 망연절 배정(멜 8·스칼렛 12·무골 21)을
	                      #   전부 비껴간다.
	...
	"frosty": [3, 28],    # ★[S9b-T4] 프로스티 = 예티. **한 해의 맨 마지막 날**(성야절 28일) —
	                      #   프로스티는 그날 밤의 *여파에서* 난 존재라([narrative-bible §5.4]),
	                      #   "끝난 다음에 남은 것"이 그대로 날짜가 된다. 밤이 가장 긴 절기의
	                      #   마지막 밤 = 공허가 가장 큰 밤이고, **다음 날이 피안절 1일**(언 것이
	                      #   풀리는 절기의 첫날)이라 "공허가 사랑으로 채워지는" 본성 축의 달력
	                      #   판이기도 하다. 회피일(25일·성야 행사 15일)과 기존 성야절 배정
	                      #   (풀무 6·설화 14·바나 27)을 전부 비껴간다.
}
