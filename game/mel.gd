...
const LINES_WARMING := [
	"또 왔네. 단골은 손해 안 보게 해주는 게 내 신조야. …앉기나 해.",
	"장사하면서 알았어. 돈은 뜯어내는 게 아니라, 돌게 두는 거더라고.",
	"살아선 사람을 쥐어짰지. 여기선 반대야 — 따뜻한 거 한 잔 내주고, 또 오게 만드는 거.",
	"…환대. 그게 내 속죄법이야. 웃기지, 사채업자였던 내가 이런 소릴 하다니.",
]

# 하트 4+(가까운 사이): '사실 조각' 떡밥 — 멜은 카페 장부를 쥐어 옥자가 봉인한 죄목의
# *존재*를 안다(CONTEXT '메인 스토리' 멜=기록 각도). 내용은 모르고, 폭로하지 않는다.
const LINES_FACT := [
	"너한텐 말해도 되겠다. 난 이 카페 장부를 통째로 쥐고 있거든.",
	"네 계약서… 봤어. 다른 셋이랑 딱 한 군데가 달라.",
	"'죄목' 칸. 거기만 새까맣게 봉인돼 있더라. 장부 십수 년 만지면서 그런 건 처음 봤어.",
	"옥자가 직접 봉했더군. 뭘 덮었는진 나도 몰라. 그치만 거기 *뭔가* 있다는 건 확실해.",
	"언젠간 네가 직접 알게 되겠지. 난 그저, 그 칸이 빈칸이 아니란 걸 일러주는 것뿐이야.",
]

# 오늘 이미 일일 대화를 한 뒤 또 말 걸었을 때(점수 없음 — 대사만 가볍게 바뀐다).
const LINE_AGAIN := "오늘 장사는 너랑 한 걸로 충분해. 또 보자고."
