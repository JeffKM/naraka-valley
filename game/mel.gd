...
# 하트 0(온보딩): 시크·건조한 강시 운영자 소개 + 역할(카페 운영·장부) + 가벼운 속죄 암시.
# ★[폴리시 R13] 소개 두 번째 줄이 **폐기된 "멜=출하대 담당" 설정**을 그대로 말하고 있었다
#   ("출하대가 내 담당이지 … 여기로 가져오면 골드로 쳐준다" = 담당자 + 즉시 지급). ADR-0021이
#   출하대를 무인화했고(CONTEXT [출하대] — "멜은 서빙 직원으로, 출하대는 주인 없는 창구로 분리")
#   구현도 무인·익일 정산이라, 첫 대화가 바로 옆 프롬프트와 어긋났다. 멜의 축(돈·장부·정확한
#   계산)은 그대로 두고 **소유권만** 사실로 되돌린다 — 정산 시점은 아래 "첫 정산" 줄이 이미 말한다.
...
const LINES_INTRO := [
	...
	"[talk]밭에서 거둔 건 저 출하함에 넣어. 주인 없는 창구다 — 나한테 안 들고 와도 값은 정확히 매겨져.",
