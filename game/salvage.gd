...
# ── ★[폴리시 2회차] 하한 퍼밀 — "대흉엔 얇아지되 죽지는 않는다" ───────────────
# 왜 필요한가: [명부의 운](ADR-0065 결정 5 ⑤)이 이 확률에 **가산**으로 얹히는데 계수가 0.5라
#   진폭이 ±50‰다. 기본이 30‰이라 대흉(운 −0.10) 날은 30 + (−50) = −20‰ → `roll`의 `permil <= 0`
#   가지에 걸려 **인양 시스템이 통째로 정지**한다(퍼크 없는 플레이어 기준). "운이 나쁘면 덜 걸린다"가
#   아니라 "운이 나쁘면 없는 기능이 된다"는 건 가산 배선의 사고지 설계가 아니다.
# ★ 5‰ = 기본의 1/6. 걸리면 반가운 정도로 얇지만 0이 아니다.
# ★ **`roll` 안에서 클램프하지 않는다**(이중 클램프 금지). `permil <= 0`은 "이 롤을 아예 끄겠다"는
#   호출 측의 명시 의사(테스트·미배선 경로)라 그 뜻이 살아 있어야 한다 — 하한은 *운이 섞이는 파생
#   지점 한 곳*(main.fishing_salvage_permil_today)에서만 적용된다.
# ★ [삽사리] 대흉 완화(DailyLuck.FLOOR_MITIGATED)와 **위상이 다르다**: 저건 운 *값*의 하한이라
#   대흉을 흉으로 한 눈금 올리고, 이건 그 값이 퍼밀로 환산된 *뒤*의 하한이다. 둘이 겹쳐도 이중
#   보정이 아니다 — 삽사리 만점(운 −0.069 → 30−35 = −5‰)도 클램프 전엔 여전히 음수라, 이 하한이
#   없으면 삽사리를 다 채운 플레이어의 인양도 대흉 날 똑같이 멈춘다.
const MIN_PERMIL := 5

# 운이 섞인 퍼밀에 하한을 얹는다(파생 지점의 단 한 곳에서만 부른다).
static func clamp_permil(permil: int) -> int:
	return maxi(permil, MIN_PERMIL)
