...
const CATALOG := {
	...
	NARU_VILLAGE: {
		...
		#   C1 치수 일반화 덕에 size만 키우면 빌드 경로·카메라가 따라온다. main의 강(RIVER_X 49·50)·
		#   다리(BRIDGE_Y 36)·8채 외관·실내 띠(카페·집·만물상 +48)·동선·라벨·워프 at도 함께 재배치됨.
	...
	#   황천해(미확대)는 불변. spawn=(28,38)(남단 나룻터). 하구(→황천해) at=(54,20)(동단 가장자리).
	SAMDOCHEON: {
		"name_ko": "삼도천",
		"size": Vector2i(56, 40),     # ★C4 = main.MAP_W' × main.OUTDOOR_H' (56×40 낚시 무대)
		"spawn": Vector2i(28, 38),    # 남단 나룻터(나루 마을 → 삼도천 도착 칸) — ★C4 남단 중앙
		# 나루 마을 ──(나룻터)── 삼도천 ──(하구)── 황천해.
		"warps": [
			{"to": NARU_VILLAGE, "at": Vector2i(28, 39), "dest": Vector2i(52, 2)},  # 남단 나룻터 → 나루 마을(dest=마을 안, 불변)
			{"to": HWANGCHEONHAE, "at": Vector2i(54, 20), "dest": Vector2i(2, 15)},  # 동단 하구 → 황천해(★C5: 64×44 새 spawn 2,15)
		],
	},
	...
	#   워프는 하나(삼도천 복귀). 부두·바다 낚시터는 무대만(라벨), 생선가게 서비스는 후속. spawn=(2,15) 서단.
	HWANGCHEONHAE: {
		"name_ko": "황천해",
		"size": Vector2i(64, 44),     # ★C5 = main.MAP_W' × main.OUTDOOR_H' (64×44 넓은 바다 무대)
		"spawn": Vector2i(2, 15),     # ★C5 서단(삼도천 하구 → 황천해 도착 칸) — land 영역 중앙부
		# 삼도천 ──(하구)── 황천해(막다른 바다 무대).
		"warps": [
			{"to": SAMDOCHEON, "at": Vector2i(1, 15), "dest": Vector2i(53, 20)},  # ★C5 서단 → 삼도천 하구(dest=삼도천 안, 불변)
		],
	},
