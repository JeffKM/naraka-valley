...
const CATALOG := {
	...
	NARU_VILLAGE: {
		...
		"warps": [
			...
			{"to": SAMDOCHEON, "at": Vector2i(52, 1), "dest": Vector2i(20, 22)},  # 북동 나룻터 → 삼도천(혼백관) — M3.1 점등
		],
	},
	...
	SAMDOCHEON: {
		...
		"warps": [
			{"to": NARU_VILLAGE, "at": Vector2i(20, 23), "dest": Vector2i(52, 2)},  # 남단 나룻터 → 나루 마을(★C3: 나룻터 52,1 한 칸 안)
