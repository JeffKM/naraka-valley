...
func _process(delta: float) -> void:
	...
	# ★[폴리시 R25 #0] **시간은 그려진 띠에게만 흐른다.** `layout`이 접은 띠(스택 위쪽 = 더 오래된
	#   항목)의 secs까지 함께 깎으면, 그 머리말이 선언한 «무손실»(앞의 띠가 시간으로 사라지면 그대로
	#   다시 선다)이 거짓이 된다 — 한 프레임에 같은 secs로 밀린 줄들은 **동시에** 만료되므로 숨은
	#   띠에는 되돌아올 프레임이 0이다. 개별로 봐도 먼저 만료되는 쪽은 언제나 «먼저 밀린 = 숨어
	#   있는» 항목이라, 접힘은 곧 소멸이었다(R11이 keep으로 축출 단계에서 막은 그 유실이 R23 #14가
	#   높이를 가변으로 만든 뒤 그리기 단계에서 그대로 재발했다 — 완공 래치처럼 재발화 경로가 없는
	#   줄은 그 세이브에서 영영 안 뜬다).
	#   ★ 큐가 멎지는 않는다: `layout`은 최신 한 줄을 예약 영역보다 커도 반드시 세우므로 매 프레임
	#     최소 하나는 시간을 쓰고, 그것이 사라지면 다음 띠가 내려온다.
	var shown: Dictionary = {}
	for slot in layout(HanjiUi.font(), _view()):
		shown[int(slot["idx"])] = true
...
#     ★[폴리시 R25 #0] 그 «무손실»의 절반은 `_process`가 진다 — **여기서 접힌 띠는 시간도 안 쓴다**
#       (안 그러면 숨은 채 만료돼 한 프레임도 안 그려진다. 그 함수 머리말에 경위).
