...
> **상태:** 스펙 확정(2026-07-05, owner grill 후속 — 타이틀 형식·탭 배선 2결정 잠금), owner Gemini 수동 생성 대기([ADR-0025] 게이트).
...
- 툴팁: 스타듀식 호버 시 `hud_tooltip`에 탭 한글명 표시(아이콘만이라 첫 사용자 학습 보조 — ❓ §7-b).
...
> **★ 이 §2는 nice-to-have.** 탭·타이틀(§1·§3)보다 우선순위 낮음. owner가 §7-c에서 "지금/후속"을 정한다. 스펙만 먼저 박제.
...
> **owner 결정(2026-07-05):** **(A) 풀 배경 일러스트.** 저승 컨셉카페 야경 손그림 배경 1장 + 로고 + [새 게임]/[이어하기] 버튼 오버레이. [okja-overview] 히어로(`screenshots/cafe-night.png`)의 카페 야경 심상을 게임 타이틀로 계승 가능.
...
```
detailed pixel-art title-screen illustration for a cozy underworld/afterlife farming game (in the style of Stardew Valley and Sun Haven title art, warm and inviting despite the afterlife setting). SCENE: a warm glowing underworld concept-cafe at dusk/night — a cozy two-story timber cafe building with big amber-lit windows spilling warm light, soft foxfire-blue (#60d8f0) paper lanterns hung under the eaves, a few red spider lilies (higanbana, #c0403a) blooming in the foreground, a hanok-tiled roofline, wisps of spirit-mist and a deep starry twilight sky above with a slim crescent moon. optional distant silhouettes of the farmstead and rolling hills. WARM cozy welcoming mood — the heart of the village at dusk, NOT scary. warm honey-amber and cream palette with cool foxfire-blue and muted crimson accents, slightly desaturated. flat 2D pixel art, painterly pixel shading, light source from the glowing cafe windows. Leave the TOP-CENTER area calmer/darker for a logo and the BOTTOM-CENTER area calmer for menu buttons. 16:9 aspect, high detail crisp readable pixels.
```

### 3.3 로고 프롬프트 (`title_logo`)
> 한글 주 타이틀 + 로마자 보조. 붓글씨(sumi-ink) + 여우불 액센트 = 게임 정체성.
```
a pixel-art game LOGO wordmark on a transparent background, the Korean title "나라카 밸리" as the large main title rendered like thick sumi-ink brush calligraphy strokes (bold, warm-dark-brown #401818 to ink-black), with a small foxfire-blue (#60d8f0) will-o-wisp flame accent nestled by the letters, and the smaller roman subtitle "NARAKA VALLEY" beneath in a clean chunky pixel serif. warm aged-hanji-paper feel, a faint scorched-edge glow behind the strokes, crisp readable pixels, single dark outline, centered, transparent bg. cozy afterlife farm game branding.
```
> ⚠️ Gemini는 정확한 한글 자형을 자주 틀린다 — 로고 글자가 어긋나면 **로고 텍스트만 owner가 폰트/Aseprite로 별도 조판**([ADR-0001] 허용 글루)하거나, `create_font`(PixelLab) 한지 폰트로 조판. Gemini 산출은 *레이아웃/장식 참조*로 쓰고 자형은 사후 교정 가능.
...
- **[새 게임]** = `saver.has_save()`면 "기존 세이브 덮어쓰기?" 2단 확인(`_delete_save_and_restart` 결) 후 신규 시작 / **[이어하기]** = `has_save()` **없으면 비활성(dim)**, 있으면 `_load_game()` 경로.
...
## 7. 열린 결정 (owner 승인 게이트 — [ADR-0025])

승인 시 함께 확정. 미해결이어도 §1·§3 프롬프트는 그대로 투입 가능(아래는 정련 포인트).

- **a. 로고 자형 처리** — Gemini 한글 자형이 어긋나면: (i) owner Aseprite 조판 / (ii) PixelLab `create_font` 한지 폰트 조판 / (iii) Gemini 재롤. **권장 = (i)**(붓글씨 로고는 손조판이 확실). 로고 텍스트 = "나라카 밸리" 주 + "NARAKA VALLEY" 보조 — 확정?
- **b. 탭 툴팁** — 아이콘만이라 첫 사용자 학습용 호버 툴팁(탭 한글명)을 넣을지. **권장 = 넣음**(스타듀 정합·저비용).
- **c. 시계 아이콘 우선순위** — §2(절기/시간대 8장)를 이번 큐에 넣을지 후속 서랍으로 뺄지. **권장 = 후속**(탭·타이틀 먼저 — 시계는 텍스트로 이미 읽힘).
- **d. 타이틀 캐릭터 실루엣** — `title_bg`에 미호·멜·바나·옥자 원경 실루엣을 넣을지(정합 리스크) vs 순수 씬. **권장 = 순수 씬 + 원경 실루엣 옵션**(§3.2 "optional distant silhouettes").
- **e. 세이브 덮어쓰기 UX** — [새 게임] 시 기존 세이브 2단 확인 문구/흐름(§3.4). 배선 세션에서 확정.
