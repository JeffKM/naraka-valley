...
## 25. ★[폴리시 2회차 · 시각/아트 배치] 실내 창구·진열·게시판 — 프롭 12·아이콘 2 스펙카드

> 이 절은 **아트 패스가 아니라 폴리시 패스**의 산출이다. 새 시스템이 아니라, 전 구역 덤프 재판정
> (vis-judge-regions2, 2026-08-20)이 "평면 검정/회백 rect + 단색 원점"으로 읽힌다고 지목한 자리들을
> 메운다. 그래서 카드마다 **"1차 판정이 무엇을 보았나"**가 먼저 적혀 있다 — 다음 사람이 같은
> 자리를 다시 판정할 때 기준이 그 문장이다.

### 25.0 공통 규약

```
전부 [ADR-0050] 32-native · [§1.1] NW 광원 · [§8.1] 하드 알파 · [§9] 저승 muted · [§3] 발치 앵커.
생성: create_image_pro(no_background=true · style_image_base64 = 그 방의 기존 프롭 1장) —
      §24.0의 pixen 호출과 갈린 이유는 **한 방에 이미 선 아트가 있기 때문**이다. 스타일 레퍼런스로
      그 방의 프롭을 물리면 팔레트·외곽선·계단 음영이 처음부터 맞아 후처리 계수가 덜 필요하다.
      (나락 = narak_seal.png · 혼백관 = museum_shelf.png · 시련장 = trial_stall.png 12색 양자화)
      ⚠️ style_image_base64는 **1.3KB 넘으면 전송 중 잘린다**(codex_stand 1656자 → 1241바이트 실측
        절단). 레퍼런스는 작은 파일을 고르거나 quantize(colors=12, FASTOCTREE)로 줄여서 물린다.
muted 계수(§24.0의 값을 그대로 물려받는다 — 새 계수를 세우지 않는다):
  아이콘        0.90/0.97 · 월드 프롭 0.85/0.95 · 혼백관 진열 0.68/0.88
후처리 글루: tools/make_polish_r2_art.py (raw에서 매번 새로 굽는 멱등 — 최종 PNG 재입력 금지).
★청키화(enforce_chunk)는 걸지 않는다 — 기존 프롭 코퍼스가 전부 1px 고움(측정 0~20%)이라,
  새것만 청키화하면 그 프롭만 그레인이 튄다(§24.0이 그은 선과 같은 결론, 이번에 수치로 재확인).
★★ 상태를 아트에 굽지 않는다 + **코드가 계속 그린다**(§24.10 ③의 물그릇 교훈이 이 패스의 축):
   건축 중 작업물 · 게시판 쪽지/도장 · 좌대에 얹힌 유품·레어크로우 · 상자 "보관 중" 표식 ·
   매대 잔고 패. 전부 원장 파생 코드가 아트 **위에** 그린다. 회귀가 소스로 이것을 잰다
   (playtest/polish_r2_art_test.gd ⑤ — 아트 분기와 상태 드로우 사이의 early return 금지).
```

### 25.1 ★길드·목공방 실내 창구 6종 (직전 세션 raw 유산 — 이번에 후처리·배선)

```
`props/guild_counter.png` (192×32) · `props/guild_weapon_rack.png` (64×32)
`props/woodshop_counter.png` (192×32) · `props/woodshop_workbench.png` (64×32)
`props/woodshop_logs.png` (64×32) · `props/woodshop_workpiece.png` (32×32)

1차 판정: "길드/목공방 카운터 = 회색 막대 · 무기 걸이 = 선 그림 · 작업대 = 선 그림"(기지 항목).
생성: 0 — raw 6장이 직전 세션(2026-08-15)에 이미 생성돼 `props/raw/`에 미추적으로 남아 있었다.
배선: `_draw_woodshop_room`·`_draw_guild_room`에 `_prop_tex` 우선 분기(폴백 = 옛 draw_rect 전량 보존).
★★ **카운터 폭 = 응대 줄 폭**(6칸 = 192px). 단품 32px를 6번 붙이면 마구리가 여섯 벌 서서
   "작은 카운터 여섯 개"로 읽힌다(실측) — 글루의 `widen()`이 좌우 6px 마구리만 남기고 그 사이
   몸통을 반복해 채운다. 드로우는 **폭이 안 맞으면 폴백으로 떨어진다**(늘어난 그림보다 옛
   통짜 판이 낫다) → 응대 줄 좌표(GUILD_COUNTER_X0/X1·WOODSHOP_COUNTER_X0/X1)를 고치면 이 PNG도
   같이 다시 구워야 한다. 회귀가 그 결합을 좌표 상수에서 **파생해** 잰다(192 하드코딩 금지).
★★ `woodshop_workpiece`는 **작업대와 따로 굽는다** — 건축 의뢰가 도는 동안에만 상판에 얹힌다.
   작업대에 구우면 아무 의뢰도 없는 날에도 목재가 놓여 있다.
★ `guild_weapon_rack.png`은 S5-T9판(가는 선 그림)을 **덮어썼다**. 같은 64×32라 배선 무변경.
   옛 raw는 `props/guild_weapon_rack_raw.png`(props/ 안에 잘못 놓인 자리)에 그대로 남아 있다.
```

### 25.2 ★혼백관 진열 2종 `props/{museum_pedestal,museum_donate_table}.png` (32×20 · 64×32)

```
1차 판정: "반딧넋 안치대 열이 무늬·음영 0의 평면 검정 직사각 다수(2줄 ≈14개)로 그려지고,
  그 위 반딧넋이 단색 원점(노랑/초록/청록/청/보라/자홍)으로만 얹힌다. 벽면 검정 사각 3개도 동계열."
★ 이 판정의 **정체를 먼저 밝혀 둔다**(다음 판정자가 같은 오독을 안 하도록): 그 "안치대 열"은
  안치대가 아니다. 진짜 안치대(firefly_stand)·열람대(codex_stand)는 S10-T9 아트가 이미 서 있다.
  판정자가 본 것은 ㉠ 유품 좌대 3좌(draw_rect 20×12) ㉡ 책 서가 8좌(museum_shelf — 아트 있음)
  ㉢ 레어크로우 진열 8좌(draw_rect 20×10) ㉣ 기증대(draw_rect 64×20)이고, "단색 원점"은
  반딧넋이 아니라 **레어크로우 머리 색 원**(`Color.from_hsv(i/8, …)` 무지개)이었다.

`museum_pedestal.png` (32×20) — **유품 줄과 레어크로우 줄이 공용한다**(생성 1로 두 줄).
  정체성: 돌 대좌 + 목재 상판 + 정면 놋빛 띠. 위가 **비어 있어야** 한다(얹히는 것은 코드가 그린다).
  배선: 20폭 슬롯 위에 32폭을 가운데 맞춰(-6) — 책 서가(museum_shelf)가 이미 쓰는 그 규약.
`museum_donate_table.png` (64×32) — 기증대. 나무 상판 + 다리 둘 + 가운데 흰 천 러너 + 빈 헌납 쟁반.
★★ **좌대에 물건을 굽지 마라.** 유품 3종·레어크로우 8종은 원장(museum.donated·_rarecrow_owned)
   파생으로 코드가 좌대 **위에** 그린다 — 빈 좌대와 찬 좌대가 같은 그림이면 진열이 진열이 아니다.
★ "단색 원점" 해소는 아트가 아니라 **배선**으로 했다: 색 원 대신 `_item_icon(rid)`가 주는
  `rarecrow_N_icon.png`(밭·인벤이 이미 쓰는 그 파일)를 좌대 위에 세운다 — [§15] 아이콘 공용
  규약의 이행이고 생성 0이다. 유품 줄도 같은 경로(아이콘 없으면 옛 색 다이아로 폴백).
```

### 25.3 ★나락 아레나 `props/narak_shaft.png` (32×32) + 봉인 고리(**재사용 — 생성 0**)

```
1차 판정: "깨진 봉인 고리가 곡률이 전혀 없는 평면 검정 직사각 밴드 2개로 그려지고, 그 사이가
  누출구다. 봉인석(보라 마름모)만 도트고 고리 본체는 무늬·음영 0의 rect-fill. 하강 구멍도 식별 불가."

㉠ 고리 본체 = **`assets/tiles/wall.png`(심리스 절석 벽돌)를 그대로 재사용한다. 생성 0.**
   원인은 아트 부재가 아니라 **ROCK 셀에 SOLID_TEX가 없다**는 것이었다(= COLORS[ROCK] 단색 칠).
   봉인 고리는 쌓아 올린 담이고, 이 세계의 담은 다 같은 절석이다. 밴드 톤은 `_mine_cast(1.0)`이
   그리기 시점에 얹으므로 아레나 자보라가 자동으로 물든다("돌 = 암반과 한 재질이니 같은 빛을
   받는다"는 갱도 돌의 그 규율 1:1) → 파생 PNG 0장.
   ⚠️ **재시도 금지 이력**: `create_image_pro`로 32² 암반 타일을 뽑으면 **캔버스를 안 채운다**
      (여백 있는 벽돌 패널이 나와 타일링 시 격자 틈이 보인다 — 후보 64장 전량). `create_tiles_pro`
      (square_topdown/segmentation)는 심리스하지만 **평면 자주색에 실금 몇 줄**뿐이라 석재로 안 읽힌다
      (16종 전량). 40 gen을 여기 태우고 재사용으로 돌아섰다.
   아트를 나중에 굽는다면 파일명은 `props/narak_ring_stone.png`(32² 심리스) — 드로우가 이미 그
   이름을 먼저 본다.

㉡ `narak_shaft.png` (32×32) — 하강 구멍([F] = 런 시작).
   정체성: 돌바닥이 깨져 열린 **아가리**. 금 간 자보라 암석 이빨이 테두리를 두르고 안은 바닥 없는
   칠흑, 안쪽 가장자리에만 찬 보랏빛 실선.
   ★★ **밴드 톤을 안 얹는 유일한 나락 프롭**이다. [§17]이 "게임플레이 중요 오브젝트는 명도로
      배경에서 뜬다"를 요구하고, 이 칸은 [F]를 누르는 자리다 — 심연 자보라 위에서 테두리가 안
      읽히면 구멍이 아니라 얼룩이 된다(1차 판정이 정확히 그것이었다).
   ★ 사다리·밧줄을 그리지 마라 — 내려가는 수단이 아니라 **떨어지는 입**이다.
```

### 25.4 ★시련 게시판 v3 `props/trial_board.png` (32×32) — 판면을 먹빛으로

```
1차 판정: "게시판이 무늬·글자·틀이 전혀 없는 회백색 사각(모서리만 접힌 형태)."
원인: v2 아트의 **판면이 밝은 회백**이라, 그 위에 얹히는 흰 쪽지(10×11 draw_rect)가 판에 묻혀
  둘이 한 덩이로 보였다. 판면 명도를 내리는 것이 해법이지 쪽지를 키우는 것이 아니다.
정체성: 두꺼운 풍화 목재 틀 + **먹빛 옻칠 판면** + 네 모서리 쇠못 + 상단 가로대의 찬 보랏빛 광택.
  판면은 **완전히 비운다**(글씨·종이 금지 — 걸리는 것은 코드가 그린다).
★★ 이 판이 **길드 토벌 게시판에도 그대로 쓰인다**(생성 0 재사용). 그 판은 정의상 **빈 판**이라
   (ADR-0063 결정 6 "위상만 예약") 걸리는 쪽지가 없다 — 한 장이 두 자리를 덮는다.
★ 곁들여 **쪽지 그림도 코드에서 고쳤다**(`_draw_pinned_note` 신설): 옛 흰 사각 하나 → 종이 + 먹줄
  두세 줄 + 압정(+ 수락 시 붉은 도장). 시련장·만물상 두 게시판이 같은 함수를 쓴다.
```

### 25.5 ★가구 세트 매대 아이콘 `ui/deco_{jaetnun,jeoseungsol}.png` (각 32×32)

```
1차 판정: "시련패 매대 2행 '잿눈 가구 세트' 아이콘이 흰 빈 정사각(색박스)." **high**
원인: 가구 세트는 **인벤 아이템이 아니라** ItemCatalog 아이콘 표에 없고, 매대 행이 지금까지
  `swatch`(세트 대표색 사각) 하나로 버텼다. 잿눈 대표색이 회백 0.72라 한지 판 바탕과 안 갈렸다.
생성: 0 — raw 2장이 직전 세션 유산으로 `ui/raw/`에 있었다. 세트 대표 가구를 그린 그림이다
  (잿눈 = 흰 침구 침대 · 저승솔 = 목재 탁자).
배선: `_deco_icon(set_id)` 신설(파일명 = 세트 id 소문자) → 매대 행에 `icon_tex`. **swatch 폴백은
  전 행에 그대로 남는다** — 세트 4종 중 아트가 온 둘만 갈아 끼워도 나머지 둘(혼불·피안화)이 안
  깨진다. 야시장·목공방·보부상 매대도 같은 경로를 탄다(한 자리에 배선하면 넷이 함께 낫는다).
★ 남은 2종(`deco_soulfire.png` · `deco_higanbana.png`)이 **다음 큐**다 — 파일만 놓으면 코드 0줄.
```

### 25.6 ★저장 상자 `props/storage_chest.png` (32×26) — 집·갈무리방 공용 한 장

```
1차 판정: "집 실내 북벽에 무늬 없는 갈색 사각(테두리만 있는 빈 액자형) 1개."(low·확인 필요)
정체: **집 저장 상자**(CHEST_TILE (11,68))의 그레이박스였다 — 궤짝 몸통 + 뚜껑 띠 + 놋빛 걸쇠.
정체성: 닫힌 호두목 궤짝. 쇠 띠 둘 + 정면 걸쇠 + 모서리 쇠붙이 + 뚜껑 앞모서리 밝은 하이라이트.
★★ 한 장이 두 자리를 덮는다(집 CHEST_TILE · 갈무리방 STOREHOUSE_CHEST_TILE — 같은 궤짝이다).
★★ **"보관 중" 밝은 점은 굽지 마라** — 넣어 둔 게 있으면 코드가 뚜껑 위에 얹는다(출하함과 같은 결).
```

### 25.7 이 패스가 바꾼 렌더 (아트 생성물 아님 — 코드)

```
① `_prop_tex` 우선 분기 신설 9자리 — 길드 카운터·목공방 카운터/작업대/원목/작업물·혼백관 좌대/
   기증대·나락 하강 구멍·저장 상자. 전부 폴백(옛 draw_rect) 보존.
② `_draw_narak_mouth` — ROCK 띠 위에 절석 타일을 덮어 깐다(그리드·충돌 무접촉 · `_mine_cast(1.0)` 틴트).
③ `_draw_pinned_note` 신설 — 시련장·만물상 두 게시판이 공유하는 쪽지 그림.
④ 혼백관 유품·레어크로우 진열이 **아이콘 공용 경로**(`_item_icon`)를 탄다(색 원·색 다이아 → 실물).
⑤ `_deco_icon` 신설 + 매대 4경로에 `icon_tex` 배선(inv_frame는 있으면 그림·없으면 swatch).
⑥ `inv_frame` 매대 행 — 이름을 `draw_text_fit`으로(말줄임 전에 한 단 줄인다) · 시련패 매대만
   이름 칸 150→180 / 가격 칸 156→186(값이 한두 자리라 [구매] 버튼을 안 건드린다) · 행 기하
   리터럴을 상수로(회귀가 그 치수로 겹침을 기하 계산해 잰다).
⑦ 길드 토벌 게시판이 시련 게시판 아트를 재사용.
★ **판정·원장·경제 로직 0줄.** 드로우 경로와 아이콘 조회만 만졌다.
```

### 25.8 이 패스가 **안 만든** 것 (이월 — 사유를 남긴다)

```
① **먹갈기 남향(정면)까지 가림** — §24.11 ⑤가 "북향만"으로 적어 둔 것이 **틀렸다**. 시트를 실측하니
   row0(정면)·row1(후면) 둘 다 콘텐츠 폭이 14px(정면에서 본 말 가슴/엉덩이)이고 기수 스프라이트가
   ≈20px이라, **위·아래로 달릴 때 말이 통째로 안 보인다**(좌·우는 정상 — 4방향 재캡처로 실증).
   ★ 1차 판정의 "4방향이 사실상 1방향"은 **하네스 결함**이었다(아래 25.9 ㉠) — 행 매핑 코드는
     처음부터 정상이었다. 남은 진짜 결함은 이 **폭**이고, 근본 해법은 §24.11 ⑤가 적은 그대로
     "말·기수를 한 장에 함께 구운 승마 시트"다(플레이어 의상까지 아트에 굽는 결정 = 권한 밖).
   차선(더 싼 길): 정면·후면 행만 **말 폭을 24px 이상으로** 다시 굽는다. 말이 기수보다 옆으로
   넓어야 "탄 것"이 실루엣으로 성립한다 — 좌·우 행이 이미 그 폭이다.
② **가구 세트 아이콘 2종**(혼불·피안화) — 파일만 놓으면 코드 0줄(§25.5).
③ **혼백관 책 서가**(museum_shelf 32×12) — 아트가 이미 있으나 어두운 가로 막대라 판정자가 좌대와
   같은 계열로 읽었다. 세 줄(유품·책·레어크로우)이 서로 다른 진열대여야 위계가 읽힌다.
④ **광맥 종 식별**(유철·혼탄이 구분 불가한 동일 암회색 상자 — 1차 판정 medium) — 이 패스의
   스코프 밖. 색이 아니라 실루엣을 갈라야 하는 문제라 카드가 따로 필요하다.
```

### 25.9 육안 판정면 — 하네스 결함 3건 교정 (★이 패스의 절반은 여기였다)

```
1차 판정 12건 중 **4건이 아트가 아니라 하네스 결함**이었다. 판정면이 틀리면 없는 결함을 고치고
진짜 결함을 놓친다 — 그래서 하네스 교정을 아트와 같은 무게로 적어 둔다.

㉠ **먹갈기 4방향이 전부 좌향으로 찍혔다**(s10_art_dump ⑩). 원인: ADR-0024대로 `_update_target`이
   매 프레임 커서 쪽으로 `face_toward`를 부르므로, 하네스가 `player._facing`에 직접 쓴 값이 그
   프레임에 곧바로 덮인다. 커서를 옮기는 길은 **막혀 있다**: 창이 비포커스라 `Input.warp_mouse`도
   `Input.parse_input_event`도 `get_global_mouse_position()`을 안 움직인다(둘 다 실측).
   → 해법 = 방향 캡처 직전에 `m.set_process(false)`로 커서 축을 통째로 끈다(정지 화면 grab이라
     잃는 것이 없다). 이 블록이 덤프의 마지막이라 되살리지 않는다.
㉡ **삼도천 물가에 반딧넋이 하나도 없었다**(⑤). 원인: ③ 혼백관 단계가 게이트 문턱(30)까지 안치하는데
   `FireflySouls.all_ids()` 앞 30개에 삼도천 고정 4자리가 통째로 들어 있어 `live_tiles`가 빈다.
   → 해법 = **⑤를 ③보다 먼저** 잡는다(순서 교체).
㉢ **결정기가 안 보였다**(⑤). 원인: 삼도천 남단은 맵 끝이라 카메라가 아래로 물려 **y38 줄이 항상
   핫바 HUD 뒤**에 깔린다(플레이어를 북으로 물려도 클램프 때문에 안 바뀐다).
   → 해법 = 결정기를 물가 스폿과 같은 y37 줄로. 앵커도 x22로(x21에 서면 몸이 스폿을 덮는다).
   ※ **팬닝 스폿 2개는 처음부터 정상 렌더였다** — panning_spot 아트가 금빛 알갱이 박힌 젖은 자갈
     덩이라 판정자가 배경 바위로 읽었다(미배선 아님).
㉣ **나락 하강 구멍이 안 보였다**(mine_dump). 원인: 고리 상변(y6..8)과 구멍(y26)은 20칸 넘게 떨어져
   한 화면(세로 11칸)에 절대 같이 안 든다. 헤더는 둘 다 보인다고 적어 두고 아레나 한 장만 잡았다.
   → 해법 = `mine_narak_shaft` 컷 신설(중앙 y24 앵커).

★ 덤프는 **한 번에 하나씩** 돌린다. 세 하네스를 한 셸 루프로 연달아 돌렸더니 두 번째(mine_dump)가
  `_descend_mine` 실패로 죽었다(`_mine_layout["rect"]` 없음) — 단독 재실행은 즉시 정상. save.dat
  전역 공유의 같은 경합 계열이다.
★ 기계 판정은 `playtest/polish_r2_art_test.gd`가 든다 — 47단언. 폴백 미도달 + **상태 드로우 생존**
  (아트 분기와 상태 드로우 사이 early return 금지)을 소스로 잰다. 치수는 전부 좌표 상수·
  InventoryFrame 상수에서 파생한다(192·180·186을 테스트에 옮겨 적지 않는다).
```
