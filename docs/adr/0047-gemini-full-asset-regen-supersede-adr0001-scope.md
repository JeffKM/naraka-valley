...
- **폴백:** 특정 에셋에서 Gemini 결과가 규격을 못 맞추면 해당 에셋만 PixelLab을 폴백으로 유지한다(전면 금지 아님 — 도구는 결과로 판단).
  - **폴백 채택 사례 — 작물(2026-07-03):** 작물 스프라이트(5종×3단계=15개)는 Gemini가 아이소메트릭 각도·흙 두둑·씨앗에 열매 그리기 문제를 반복해, owner 지시로 **PixelLab `create_map_object`(top-down/no-soil)로 전환**했다. 규약: 흙/두둑 일절 없이 식물만(투명 배경), 스타듀 룩, 32×32(트렐리스 황천포도만 32×64 세로). 변환은 `game/tools/gemini_crop_to_cell.py`(raw→bbox 크롭→cell bottom-center→2px 청키)로 흡수. 트렐리스는 `main.gd` `_draw_crops`가 `CropCatalog.is_trellis()`일 때 셀 위로 1칸 솟는 32×64로 렌더.
