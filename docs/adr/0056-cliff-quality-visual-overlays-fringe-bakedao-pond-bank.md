...
> 본 ADR은 순수 설계 문서(코드 0). 실제 ①④ 코드·② 아트 재생성은 후속 빌드 슬라이스에서 [worktree-isolation-rule]대로 착수.

- [ ] **① fringe 오버레이:** `_draw_cliff_fringe`가 `CLIFF_LIP` 상단 엣지에 ragged 풀 오버레이를 비파괴로 얹고 직각 seam이 소멸(`_grid`/충돌/세이브 불변).
- [ ] **② baked AO:** `cliff_s_face/base.png` 재생성 시 하단 감쇄 + 접지 드롭섀도우 베이크, **기존 §10.2 접지 그림자를 정밀화(이중 금지)**. 코드 변경 0 확인.
- [ ] **④ 연못 뱅크 자동화:** `_build_home` 하드코딩 2행 삭제 → `_autotile_pond_siblings`가 `SPIRIT_POND_RECT` 북단에서 뱅크 2행을 유도 생성, **좌표가 하드코딩과 바이트 동일**(연못 앵커 회귀 0).
- [ ] **하얀 사각형 회귀:** 캐릭터가 절벽 밑에 밀착·좌우 이동 시 머리 위 하얀 사각형 아티팩트 0(비-헤드리스 실캡처 픽셀 검수 — 헤드리스 재현 불가).
- [ ] `game/run_tests.sh` 전체 회귀 0·부팅 클린.
