...
- **④ BASE 발치 접지 그림자 밴드(신규):** `_build_ground16`에 `CLIFF_BASE`(및 곡선 base `CLIFF_CORNER_*_B`) 바로 아래(Y+1) tan/GROUND 셀 상단에 검은 알파 감쇄 밴드(`_G16_APRON_H=7`·`_G16_APRON_MAX=0.42`, `Color.darkened`)를 오버레이(ADR-0054 건물 접지 정신). `_grid`는 순수 tan 유지(충돌·세이브 불변). 아래가 물/건물/다른 절벽이면 스킵(연못 뱅크 제외). strength 튜너블. 픽셀·dump 사인오프.
