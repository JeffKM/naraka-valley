...
- [ ] **② baked AO:** `cliff_s_face/base.png` 재생성 시 하단 감쇄 + 접지 드롭섀도우 베이크, **기존 §10.2 접지 그림자를 정밀화(이중 금지)**. 코드 변경 0 확인. *(아트 트랙 — 미착수)*
...
- **① = `_build_ground16` 인라인 fringe 블록**(별도 `_draw_cliff_fringe` 함수 불요 — 오버레이 베이크가 이미 `_build_ground16`에 있어 거기 얹는 게 DRY). `_build_path_grass_fringe`의 grass_out(dir=0) *기술*(신호 있는 물결 `_gd_h01`·`_FR_MAX`/`_FR_DEAD`·blade 팁 감광)만 재활용. 풀 소스 = `_bf_grass`(lip 자체가 '고지 풀 오버행'이라 흙-지배 마당에서도 정합). `CLIFF_LIP` 셀은 `_g16_surface`가 -1로 스킵(오버레이 투명) → drooped 풀이 밑 타일맵 lip 상단을 덮는다. `_grid`·충돌·세이브 불변.
...
- **검증:** `cliff_test` ⑦(연못 뱅크 rect 유도·폭 전체·멱등)·⑧(fringe 후 `CLIFF_LIP` 격자 불변·오버레이 베이크) 신규 + 전체 회귀 PASS.
