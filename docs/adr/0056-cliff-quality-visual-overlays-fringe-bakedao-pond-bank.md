...
## FINAL 개정 (2026-07-07) — 스타듀 대조 4항목 + 오버행 제거

owner 라이브 GL 검수 후, 스타듀 레퍼런스 정밀 대조로 절벽 시각 품질을 **4항목**으로 재정식화했다(owner 문서 "ADR-0056-FINAL-RELEASE"). ★**번호 주의:** 이 pass의 번호는 위 원래 구현(①fringe·②AO·④연못뱅크)과 **다른 축**이다 — 여기 ④는 **BASE 발치 접지**(연못 뱅크 아님).

- **① 상단 엣지(Seam)** = 위 ① fringe로 **달성**(라이브 재확인만·코드 0). 현 늘어짐 밀도 표준 확정.
- **② FACE 원근 그라데이션** = 위 ② baked AO로 **달성**(라이브 재확인만·코드 0).
- **③ 노치 직각 마감 → 곡선 코너 라운딩(신규):** `_round_south_notch(gate_x, gate_w)` — `_carve_stair_notch` 뒤에 통로 마주 벽 끝을 `CLIFF_CORNER_SE`(좌벽 동측)/`CLIFF_CORNER_SW`(우벽 서측)로 스위칭. 새 타일 ID 0·순수 그리드(세이브 불변)·코너 SOLID라 통로 폭·충돌 불변. home_full_dump 사인오프(초록 곡선 말아넣기).
- **④ BASE 발치 접지 그림자 밴드(신규):** `_build_ground16`에 `CLIFF_BASE`(및 곡선 base `CLIFF_CORNER_*_B`) 바로 아래(Y+1) tan/GROUND 셀 상단에 검은 알파 감쇄 밴드(`_G16_APRON_H=7`·`_G16_APRON_MAX=0.42`, `Color.darkened`)를 오버레이(ADR-0054 건물 접지 정신). `_grid`는 순수 tan 유지(충돌·세이브 불변). 아래가 물/건물/다른 절벽이면 스킵(연못 뱅크 제외). strength 튜너블. **★[REV5] 코너 게이트:** 곡선 base(코너)는 물러난(투명) 열엔 벽이 없어 그림자가 절벽 끊긴 곳까지 이어지던 것(owner) → 코너 PNG 최하단 행 불투명 마스크로 벽 열만 그림자(투명 열 스킵).

**★① 후속 — 절벽 top 연결(owner 라이브 "위에 타일이 안 이어짐"):** fringe(풀 tufts)만으로는 부족했다 — 근본 원인은 **고지 평지가 tan(ADR-0053 흙-지배 flip)인데 lip은 초록**이라 `tan→초록 띠→갈색`으로 초록 lip이 뜬 줄무늬로 읽힌 것. **고지(하늘 목장 NW 사각 x≤HIGHLAND_E·y≤HIGHLAND_S)를 잔디-지배로** 전환(`_g16_surface`가 고지 GROUND→surf 1) → 초록 평지→초록 lip 연속=절벽 top 자연 연결(스타듀 elevated=grassy). **저지 마당만 흙-지배 유지 → 목장=잔디 vs 농장=흙 대비**(ADR-0053 flip은 저지 한정으로 정밀화). 건물 발치 tan 패드(`_g16_near_building`)는 그대로. 순수시각·`_grid` 불변. home_full_dump 사인오프. ★**후속 톤-매칭:** 평지 잔디화 뒤 절벽 lip/코너 에셋의 초록(웜·밝음 (93,136,51))이 평지 `_bf_grass`(쿨·차분 (60,119,83))와 안 맞아 "이어지는 타일과 다른 잔디"로 읽힘 → `tools/match_cliff_grass_to_field.py`(glue·ADR-0001)로 lip·코너 4종의 grass 픽셀을 field 평균으로 평행이동(`_harmonize_grass_variants` 결·텍스처 보존). Gemini 재생성 시 재적용.

**★REV4 — 텍스처 이음매 파쇄(owner 라이브 "위쪽·양끝 코너가 주변 타일과 안 이어짐"):** 톤은 맞았으나 lip 블레이드 패턴·코너 코사인 곡선이 평지 텍스처와 이질. 색 평행이동으로는 한계 → 텍스처·형태 자체를 평지 문법으로.
- **① LIP 상단 텍스처 평지화:** `_build_ground16`에서 `CLIFF_LIP` 상단 `_LIP_GRASS_H`(18px)를 **평지 `_bf_grass`로 월드-타일링 오버레이**(위 평지 grass와 씸리스 연속) — lip 잔디 텍스처가 평지와 100% 일치. 잔디↔바위 엣지 경계는 `_LIP_EDGE_JIT` raggedness. lip 하단 바위 엣지는 남김. 고지 잔디화로 무의미해진 옛 fringe(위로 솟음) 블록을 이걸로 **대체**. 순수시각·`_grid` 불변.
- **② 코너 컨텍스트 필(REV5, owner "잔디타일이 이어졌자나 → 주변 타일 인식해야"):** 초기 REV4는 코너 물러난 영역을 *고정 grass_field*로 채웠으나, owner가 "절벽이 정면 아닐 때(노치·코너) 주변 타일을 인식해 이어져야 하는데 잔디가 이어짐"을 지적. **→ `make_cliff_corners.py`가 물러난 영역을 *투명*으로 남기고(벽만 불투명·코사인 경계 raggedness 유지), 런타임 `_build_ground16`이 그 투명 픽셀을 *이웃 셀 지형*으로 채운다**(`_corner_img` 마스크 + SW=서쪽/SE=동쪽 이웃 `_g16_surface` 샘플 → 노치 코너=tan 통로 / 맵끝=저지). 벽 픽셀은 타일맵이 그림. `_grid`·충돌·세이브 불변. home_full_dump 사인오프(노치 코너가 tan 통로로 이어짐).

**★관련 — 벽면 오버행 제거(가시성 버그 픽스 후 owner 라이브):** 남향 절벽 밑 캐릭터가 벽면에 과잉 가림 → **남향 절벽 밑 = 절벽 앞이라 안 가려져야 맞음**. `_draw_front_cliff_faces`·`_front_cliff_cells_for`·`_cliff_front_tex`·`_cache_cliff_face_cells`·`_cliff_face_cells`·`CLIFF_WALL_TILES`+`front_cliff_test.gd` 제거([cliff-tileset-spec §10.2 ⑤] 반전). 부수 효과 = front 절벽 draw 경로 소멸 → 하얀사각형 위험 원천 소멸.

- **검증:** `cliff_test` ⑨(노치 코너 라운딩·SOLID·폭 불변)·⑩(발치 밴드 = 아래 tan 그리드 불변) 신규 + 전체 회귀 PASS·부팅 클린.
